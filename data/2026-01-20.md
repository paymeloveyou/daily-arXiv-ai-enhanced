...
TL;DR: ICONIC-444是一个专为OOD检测研究设计的大规模工业图像数据集，包含超过310万张RGB图像和444个类别，旨在解决现有数据集在规模和任务复杂度方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前OOD检测研究进展受限于缺乏大规模、高质量的数据集，这些数据集需要明确定义不同难度级别（近OOD到远OOD）的OOD类别，并支持细粒度和粗粒度的计算机视觉任务。

Method: 使用原型工业分拣机采集数据，构建了包含超过310万张RGB图像、444个类别的ICONIC-444数据集。该数据集模拟真实世界任务，提供结构化、多样化的数据，并定义了四个参考任务来基准测试OOD检测方法。

Result: 提供了22种最先进的基于后处理的OOD检测方法的基准结果，为OOD检测研究提供了新的评估标准。

Conclusion: ICONIC-444填补了OOD检测研究领域的数据集空白，通过提供大规模、高质量、任务复杂度多样的工业图像数据集，支持更严格的OOD评估和算法基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器人技术日益融入建筑工作流程，其理解和响应人类行为的能力对于实现安全有效的协作至关重要。视觉语言模型（VLMs）在视觉理解任务中展现出潜力，能够在无需大量领域特定训练的情况下识别人类行为，这在标注数据稀缺且监测工人行为和情绪状态对安全与生产力至关重要的建筑领域尤为吸引人。

Method: 使用包含1,000张图像的数据集，这些图像标注了10个行为类别和10个情绪类别。通过标准化推理流程和多种评估指标（包括F1分数和准确率）评估三种VLMs（GPT-4o、Florence 2、LLaVa-1.5）的性能。进行混淆矩阵分析以识别模型在语义相近类别上的混淆情况。

Result: GPT-4o在两个任务中均表现最佳：行为识别的平均F1分数为0.756，准确率为0.799；情绪识别的F1分数为0.712，准确率为0.773。Florence 2表现中等：行为F1分数为0.497，情绪F1分数为0.414。LLaVa-1.5表现最差：行为F1分数为0.466，情绪F1分数为0.461。所有模型在语义相近类别（如团队协作与主管沟通）上都存在困难。

Conclusion: 通用视觉语言模型可为建筑环境中的人类行为识别提供基础能力，但要实现实际应用的可靠性，仍需进一步改进，如领域适应、时序建模或多模态感知等增强措施。
...
TL;DR: 研究发现ID准确率与OOD检测性能存在非单调关系，训练策略、检测器选择和OOD性能之间存在强相互依赖，没有单一最优方法


<details>
  <summary>Details</summary>
Motivation: 探索OOD检测与最大化ID准确率和泛化能力的现代训练流程之间的关联，填补该领域研究空白

Method: 采用ResNet-50架构，在56个通过不同训练策略获得的ImageNet训练模型上，对21种后处理SOTA OOD检测方法进行基准测试，并在8个OOD测试集上评估

Result: 发现ID准确率与OOD检测性能存在非单调关系：OOD性能随准确率提高先改善后下降；训练策略、检测器选择和OOD性能之间存在强相互依赖

Conclusion: 没有单一OOD检测方法在所有训练策略下都最优，需要根据具体训练流程选择合适检测器
...
TL;DR: Medical SAM3是SAM3分割基础模型的医学专用版本，通过在33个医学影像数据集上微调获得，解决了医学图像分割中的领域偏移问题，实现了通用、文本引导的医学图像分割。


<details>
  <summary>Details</summary>
Motivation: SAM3等可提示分割基础模型虽然展示了强大的泛化能力，但直接应用于医学图像分割存在三个主要限制：1）严重的领域偏移（医学影像与自然图像的差异）；2）缺乏特权空间提示（如边界框）；3）需要理解复杂的解剖结构和三维体积信息。研究发现SAM3在医学数据上性能显著下降，其竞争力主要依赖于强几何先验（如真实边界框），这促使需要进行完整的模型适应而不仅仅是提示工程。

Method: 通过在大规模、异质的2D和3D医学影像数据集上对SAM3进行完全微调，创建Medical SAM3。使用了33个数据集，涵盖10种医学成像模态，所有数据都配有分割掩码和文本提示。这种方法使模型获得领域特定的表示，同时保持提示驱动的灵活性。

Result: 在器官、成像模态和维度上的广泛实验显示，Medical SAM3相比原始SAM3取得了持续且显著的性能提升，特别是在具有语义模糊性、复杂形态和长程3D上下文的挑战性场景中。模型在医学图像分割任务上表现出更强的鲁棒性和准确性。

Conclusion: Medical SAM3被确立为医学成像的通用、文本引导分割基础模型。研究强调了在严重领域偏移下，实现鲁棒提示驱动分割需要整体模型适应，而不仅仅是提示工程。代码和模型将开源，促进医学图像分割研究的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到运动生成方法主要依赖序列级或动作级描述，缺乏细粒度的部件级运动标注，限制了其对单个身体部位的可控性。需要构建具有原子级、时间感知的部件级文本标注的数据集来提升运动生成的空间和时间控制能力。

Method: 1) 利用大语言模型的推理能力构建高质量运动数据集，包含原子级、时间感知的部件级文本标注；2) 提出基于扩散的部件感知运动生成框架FrankenMotion，每个身体部件由自己的时间结构化文本提示引导；3) 实现空间（身体部位）和时间（原子动作）的双重控制。

Result: FrankenMotion在所有为本文设置调整和重新训练的基线模型中都表现优异，能够生成训练中未见过的运动组合。构建的数据集提供了异步、语义不同的部件运动在精细时间分辨率上的标注。

Conclusion: 这是首个提供原子级、时间感知部件级运动标注并实现空间和时间双重控制的运动生成工作。FrankenMotion框架通过部件级引导实现了更好的可控性和组合能力，代码和数据集将在发表后公开。
...
Method: 1. 对多种多分类方法进行综述，包括DenseNet121等深度学习模型；2. 开发开源Web应用程序用于实际应用；3. 进行不同方法的对比测试；4. 分析所提出方法的弱点并提出改进建议。

Result: 1. 提供了多种胸部X光图像多分类方法的性能比较结果；2. 成功部署了可用的开源Web应用程序；3. 识别了现有方法的局限性；4. 代码已在GitHub上开源。

Conclusion: 该研究为胸部X光图像多分类任务提供了方法综述和实用工具，指出了现有方法的不足，并为未来改进提供了方向，有助于推动胸部疾病自动诊断技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界环境中的恶劣天气条件会严重降低自动驾驶系统的模型性能和可靠性，需要开发能够适应视觉退化的鲁棒多任务学习方法。

Method: 提出RobuMTL架构，采用混合专家方式，根据输入扰动动态选择任务特定的分层低秩适应（LoRA）模块和LoRA专家小组，实现基于输入特征的自适应专业化。

Result: 在PASCAL数据集上，相比MTL基线，单扰动条件下平均相对提升+2.8%，混合天气条件下提升高达+44.4%；在NYUD-v2数据集上，各任务平均相对提升+9.7%。
...
TL;DR: 该研究评估了五种深度学习架构在仅有150张标注图像的极端数据稀缺条件下进行树冠检测的性能，发现基于卷积的预训练模型（YOLOv11、Mask R-CNN）比基于Transformer的模型（DeepLabv3、Swin-UNet、DINOv2）泛化能力更强。


<details>
  <summary>Details</summary>
Motivation: 从航空影像中检测树冠对于环境监测、城市规划和生态系统分析至关重要。Solafune树冠检测竞赛模拟了真实世界数据标注稀缺的情况，仅提供150张标注图像的小规模不平衡数据集，这对训练深度模型而不产生严重过拟合提出了重大挑战。
...
Conclusion: 研究证实了Transformer架构在低数据环境下（没有大量预训练或数据增强）表现不佳，并且语义分割与实例分割之间的差异进一步影响了模型性能。轻量级CNN方法在有限图像数据上进行树冠检测仍然是最可靠的选择。
...
TL;DR: 提出预咨询对话框架(PCDF)，通过两个视觉语言模型模拟真实医患诊断对话，利用对话监督提升诊断准确性


<details>
  <summary>Details</summary>
Motivation: 传统AI医疗诊断主要集中于图像分析，缺乏患者报告的症状信息限制了诊断准确性，需要模拟真实诊断过程中医生询问患者症状的交互过程

Method: 提出预咨询对话框架(PCDF)，使用两个视觉语言模型：DocVLM基于图像和对话历史生成后续问题，PatientVLM使用基于真实诊断的症状配置文件进行回答，生成合成诊断对话用于微调DocVLM

Result: 临床验证表明合成症状具有临床相关性、症状覆盖性和整体真实性；基于对话的监督训练相比仅使用图像的训练带来显著性能提升

Conclusion: PCDF框架通过模拟真实症状询问过程，有效提升了诊断准确性，证明了现实症状获取在医疗诊断中的价值
...
TL;DR: IDDR-NGP：首个统一的干扰物去除方法，直接在Instant-NGP上操作，可去除多种3D场景干扰物（雪花、彩纸、落叶、花瓣等），结合隐式3D表示与2D检测器，通过LPIPS损失和多视图补偿损失优化渲染结果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常专注于特定类型的干扰物去除，缺乏统一的解决方案。需要一种能够处理多种干扰物（如雪花、彩纸、落叶、花瓣等）的通用方法，以恢复被污染的3D场景。

Method: 提出IDDR-NGP方法，结合隐式3D表示与2D检测器，设计LPIPS损失和多视图补偿损失（MVCL）联合优化渲染结果，能够从多视角污染图像中聚合信息，实现端到端训练。

Result: 构建了包含合成和真实世界干扰物的新基准数据集，实验证明IDDR-NGP在去除多种类型干扰物方面具有有效性和鲁棒性，与现有SOTA去雪方法结果相当，能准确去除真实和合成干扰物。

Conclusion: IDDR-NGP是首个统一的干扰物去除方法，能够有效处理多种3D场景干扰物，通过结合隐式3D表示与2D检测器，以及设计的损失函数，实现了高质量的3D场景恢复。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI生成视频检测领域面临两个关键限制：1) 数据集角度：现有数据集规模有限，使用过时或范围狭窄的生成模型，难以捕捉现代生成技术的多样性和快速演变；2) 基准角度：当前基准主要停留在数据集创建阶段，缺乏系统性的基础问题和深入分析。

Method: 提出AIGVDBench基准，覆盖31个最先进的生成模型和超过44万个视频，对33个现有检测器（属于四个不同类别）执行超过1500次评估，从多个角度进行8个深入分析。

Result: 通过大规模评估和分析，识别出4个新发现，为未来研究提供有价值的见解。基准开源在GitHub上，为AI生成视频检测领域的进展提供坚实基础。

Conclusion: AIGVDBench填补了AI生成视频检测领域的空白，提供了一个全面且具有代表性的基准，通过系统性的评估和深入分析，为未来研究提供了重要基础和方向。
...
TL;DR: M3DDM+通过改进训练时的掩码策略解决了M3DDM在视频外绘中因训练-推理不匹配导致的质量退化问题，在信息有限场景下显著提升了视觉保真度和时间一致性。


<details>
  <summary>Details</summary>
Motivation: M3DDM在视频外绘中面临质量退化问题，表现为空间模糊和时间不一致性，特别是在相机运动有限或外绘区域较大的挑战性场景中。研究发现这是由于训练时的随机掩码方向与推理时的一致性方向外绘要求不匹配导致的。

Method: 提出M3DDM+方法：1）在训练时对所有帧应用统一的掩码方向和宽度，确保训练与推理条件一致；2）基于预训练的M3DDM模型进行微调，保持计算效率的同时解决训练-推理不匹配问题。

Result: 实验表明，M3DDM+在信息有限场景下显著改善了视觉保真度和时间一致性，同时保持了计算效率。该方法有效解决了原始M3DDM在挑战性场景中的质量退化问题。

Conclusion: 通过统一训练和推理的掩码策略，M3DDM+成功解决了视频外绘中的训练-推理不匹配问题，在保持计算效率的同时显著提升了在信息有限场景下的生成质量。
...
TL;DR: 首次提出物理感知强化学习范式，通过Mimicry-Discovery Cycle框架在视频生成中强制执行物理碰撞规则，提升物理真实性


<details>
  <summary>Details</summary>
Motivation: 当前基于transformer的视频生成模型忽视了物理原理，特别是在刚体运动渲染方面存在严重缺陷。虽然计算机图形学和物理模拟器能轻松使用牛顿公式建模碰撞，但现代预训练-微调范式在像素级全局去噪过程中丢弃了物体刚性的概念，即使完全正确的数学约束在模型优化中也仅被视为次优条件，这从根本上限制了生成视频的物理真实性。

Method: 1. 首次引入物理感知强化学习范式，直接在视频生成模型的高维空间中强制执行物理碰撞规则，确保物理知识被严格应用而非仅作为条件；2. 将该范式扩展为统一框架Mimicry-Discovery Cycle (MDcycle)，允许大幅微调同时完全保留模型利用物理基础反馈的能力；3. 构建新的基准测试PhysRVGBench。

Result: 通过广泛的定性和定量实验全面评估了方法的有效性，验证了该框架在提升视频生成物理真实性方面的效果。

Conclusion: 提出的物理感知强化学习范式和MDcycle框架填补了transformer视频生成中物理原理应用的空白，通过直接在模型优化中强制执行物理规则而非仅将其作为条件，显著提升了生成视频的物理真实性。
...
TL;DR: CoDance提出了一种Unbind-Rebind框架，用于解决多角色图像动画中任意数量、类型和空间配置的挑战，通过解绑刚性空间对齐和重新绑定运动到目标角色来实现灵活的多主体渲染。


<details>
  <summary>Details</summary>
Motivation: 现有方法在单角色动画上表现良好，但难以处理任意数量的角色、多样化的角色类型以及参考图像与驱动姿态之间的空间错位问题。这些限制源于过于刚性的空间绑定（强制姿态与参考之间的像素级对齐）以及无法将运动一致地重新绑定到目标角色。
...
Conclusion: CoDance通过Unbind-Rebind框架有效解决了多角色图像动画中的关键挑战，能够处理任意数量、类型和空间配置的角色，为灵活的多主体渲染提供了创新解决方案。
...
TL;DR: 提出了一种结合图平滑模块和增强局部几何学习模块的新方法，用于解决3D点云分析中图结构在边界点和连接区域的稀疏和噪声连接问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于图的方法在3D点云分析中虽然有效，但存在图结构次优的问题，特别是在边界点处连接稀疏，在连接区域存在噪声连接，这限制了方法的性能。

Method: 提出两个核心模块：1) 图平滑模块优化图结构，减少不可靠稀疏和噪声连接的负面影响；2) 增强局部几何学习模块，基于优化后的图结构，结合自适应几何描述子（基于特征向量）的形状特征和圆柱坐标变换得到的分布特征来改进特征提取。
...
TL;DR: ESD是一个超轻量级的30米全球地球嵌入数据库，通过将多传感器观测数据压缩为信息密集的量化潜在向量，实现340倍数据体积缩减，使全球陆地表面单年数据仅需2.4TB，支持在标准工作站上进行十年尺度全球分析。


<details>
  <summary>Details</summary>
Motivation: 卫星地球观测系统产生了海量数据，但巨大的计算和存储需求阻碍了全球尺度分析的广泛应用，限制了行星尺度研究的发展。需要解决这些技术障碍以促进地球科学研究的民主化。

Method: 使用ESDNet架构和有限标量量化(FSQ)技术，将Landsat系列(5,7,8,9)和MODIS Terra的高维多传感器观测数据转换为信息密集的量化潜在向量，将年度物候周期压缩为12个时间步长，构建统一的潜在空间表示。

Result: 实现了约340倍的数据体积缩减，全球陆地表面单年数据仅需约2.4TB；重建保真度高(MAE:0.0130; RMSE:0.0179; CC:0.8543)；土地覆盖分类准确率达79.74%，优于原始反射率数据的76.92%；具备稳健的少样本学习能力和时间一致性。

Conclusion: ESD通过革命性的数据压缩和表示方法，为民主化行星尺度研究和推进下一代地理空间人工智能提供了多功能基础，使全球尺度分析能够在标准本地工作站上实现。
...
TL;DR: 提出基于Rectified Flow的多模态联合推理算法，通过结构对齐视角实现配对样本的协同生成，相比现有方法计算更快、质量更高


<details>
  <summary>Details</summary>
Motivation: 现有方法在联合生成配对样本时缺乏结构对齐视角，而基于Score Distillation Sampling的3D生成方法存在计算耗时、模式崩溃和卡通化结果等问题，需要更高效的结构对齐生成方法

Method: 提出基于Rectified Flow的多模态联合推理算法，利用样本空间中片段的联合传输实现结构对齐，可在结构化潜空间上构建，支持图像、视频和3D形状生成

Result: 在图像、视频和3D生成任务中表现出色：实现高度结构对齐的样本对和高视觉质量，在图像和视频生成上达到SOTA，3D生成质量相当但速度快几个数量级

Conclusion: 提出的基于Rectified Flow的结构对齐联合推理方法在多个模态上优于现有方法，实现了高效、高质量的配对样本生成，为多模态联合生成提供了新思路
...
Result: 在9个图像分类数据集（包括自然图像和医学影像）上评估，BioTune在准确率和效率上均优于AutoRGN和LoRA等最先进微调方法，并在4种不同CNN架构上保持最佳性能。

Conclusion: BioTune通过进化优化实现自适应微调，能有效处理源域与目标域差异，在各种数据特性和分布变化中表现出优越的适应性和灵活性。
...
TL;DR: 提出无监督多场景行人重识别任务，通过三阶段图像-文本知识建模框架，利用视觉语言模型跨多个场景（跨分辨率、换装等）进行统一学习。


<details>
  <summary>Details</summary>
Motivation: 现有行人重识别方法通常针对单一场景设计，缺乏跨多个不同场景的统一框架。需要开发能够同时处理跨分辨率、换装等多种场景的无监督方法，以增强模型的通用性和适应性。
...
Conclusion: 提出的无监督多场景行人重识别框架通过图像-文本知识建模有效利用了视觉语言模型的表征能力，为跨多种场景的统一行人重识别提供了有效的解决方案，展示了多场景知识整合的优势。
...
Method: 开发开源本地框架SAMannot，集成Segment Anything Model 2，通过修改SAM2依赖、实现处理层降低计算开销，采用持久实例身份管理、自动"锁定-精修"工作流、基于掩码骨架化的自动提示机制

Result: 框架能生成YOLO和PNG格式的研究就绪数据集及结构化交互日志，在动物行为追踪和LVOS、DAVIS基准数据集子集上验证有效，提供可扩展、隐私保护、经济高效的视频标注方案

Conclusion: SAMannot为复杂视频标注任务提供了商业平台的替代方案，实现了本地化、高效、隐私安全的视频实例分割工作流
...
<details>
  <summary>Details</summary>
Motivation: 超高分辨率遥感图像分割在航空测绘和环境监测中至关重要。传统Transformer模型在处理UHR图像时面临内存随token数量呈二次方增长的问题，这限制了上下文范围或空间分辨率。需要一种既能捕获长距离依赖又能保持细粒度细节的解决方案。

Method: 提出CASWiT（Context-Aware Stage-Wise Transformer）架构：1）双分支设计：上下文编码器处理下采样邻域捕获长距离依赖，高分辨率编码器提取UHR补丁的详细特征；2）跨尺度融合模块：结合交叉注意力和门控特征注入，用上下文信息丰富高分辨率token；3）SimMIM式预训练：掩蔽75%的高分辨率图像token和对应的低分辨率中心区域，训练共享双编码器和小解码器重建UHR原始图像。

Result: 在大规模IGN FLAIR-HUB航空数据集上，CASWiT达到65.83% mIoU，比RGB基线提升1.78个百分点。在URUR数据集上，达到49.1% mIoU，比当前SOTA提升+0.9%（按官方评估协议）。

Conclusion: CASWiT通过双分支架构和跨尺度融合有效解决了Transformer在UHR图像分割中的内存限制问题，结合SimMIM预训练策略显著提升了分割性能，在遥感图像分割任务中达到了新的SOTA水平。
...
TL;DR: 提出了一种结合视觉语言模型与传统计算机视觉方法，通过显式逻辑推理增强情境感知的方法，包括细粒度事件细节提取、智能微调策略和推理时生成解释
...
Method: 将视觉语言模型与传统计算机视觉方法通过显式逻辑推理相结合，实现三个关键增强：(a) 提取细粒度事件细节；(b) 采用智能微调策略，比无指导选择获得显著更高的准确性；(c) 在推理过程中为VLM输出生成解释

Result: 智能微调机制提高了准确性，并在推理过程中提供了有价值的手段，既可以确认VLM输出的有效性，也可以指出其可能存在的问题

Conclusion: 通过集成视觉语言模型与传统计算机视觉方法，并引入显式逻辑推理，能够显著增强情境感知能力，特别是在细粒度事件识别、准确性提升和输出可解释性方面
...
TL;DR: 提出融合无人机和街景图像的双模态机器学习框架，用于评估全球南方城市建筑热暴露风险，识别建筑材料与热健康风险的关系。


<details>
  <summary>Details</summary>
Motivation: 气候变化加剧了全球南方城市密集建成区的人类热暴露风险，但缺乏可扩展的方法来评估与热相关的建筑属性。低成本的建筑材料和高温热质量表面进一步加剧了这一风险。

Method: 提出机器学习框架，融合公开可用的无人机(UAV)和街景(SV)图像，通过耦合全局上下文视觉变换器(CGCViT)学习城市结构的热相关表示。使用HotSat-1的热红外(TIR)测量来量化建筑属性与热相关健康风险的关系。

Result: 双模态跨视图学习方法比最佳单模态模型性能提升高达9.3%。建筑周围植被存在(相对于无植被)、较亮的屋顶(相对于较暗屋顶)、以及混凝土/粘土/木材屋顶(相对于金属或防水布)都与较低的HotSat-1 TIR值显著相关。在坦桑尼亚达累斯萨拉姆市的部署展示了如何识别家庭层面的热暴露不平等。

Conclusion: 无人机和街景图像为城市结构提供了有价值的互补视角。研究结果表明，本地化、数据驱动的风险评估在制定公平的气候适应策略中具有关键作用，能够识别与建筑材料相关的社会经济劣势导致的热暴露不平等。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在长视频理解方面受限于计算约束和次优的帧选择策略，需要更高效的框架来提升性能。

Method: TCS包含两个核心组件：(1) 多查询推理：生成多个查询以捕捉问题和视频的互补方面；(2) 片段级慢快采样：自适应平衡密集局部细节和稀疏全局上下文。

Result: 在MLVU、LongVideoBench和VideoMME数据集上的实验表明，TCS能持续提升不同MLLM的性能，最高提升6.9%准确率，且能以50%的推理时间成本获得可比精度。

Conclusion: TCS框架在长视频理解任务上展现出高效性和有效性，通过创新的查询生成和采样策略解决了现有方法的计算限制问题。
...
TL;DR: HUG提出了一种异构不确定性引导的组成图像检索范式，通过细粒度概率学习框架和异构不确定性估计，解决了CIR中内在噪声和不确定性带来的鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: CIR三元组中的内在噪声会导致内在不确定性，威胁模型鲁棒性。现有的概率学习方法存在局限性：采用实例级整体建模，对查询和目标进行同质处理，无法有效处理CIR任务。

Method: 提出异构不确定性引导（HUG）范式：1）使用高斯嵌入表示查询和目标，捕捉细粒度概念和不确定性；2）为多模态查询和单模态目标定制异构不确定性估计；3）捕获单模态内容质量和多模态协调的不确定性，通过可证明的动态加权机制计算综合查询不确定性；4）设计不确定性引导的目标函数，包括查询-目标整体对比和细粒度对比，配合全面的负采样策略。

Result: 在基准测试中，HUG超越了现有最先进基线方法，实验证明了其有效性，并通过可靠分析验证了技术贡献。

Conclusion: HUG通过异构不确定性引导的细粒度概率学习框架，有效解决了CIR中的内在噪声和不确定性问题，提升了模型的鲁棒性和检索性能。
...
TL;DR: SUG-Occ：一种利用语义和不确定性引导稀疏学习的3D语义占据预测框架，通过利用3D场景固有稀疏性减少冗余计算，在保持几何和语义完整性的同时提升效率


<details>
  <summary>Details</summary>
Motivation: 3D语义占据预测作为自动驾驶全场景理解的关键任务，提供了体素级语义信息，但现有方法存在计算和内存开销过大的问题，阻碍了实际实时部署。需要一种既能保持几何和语义完整性又能显著提升效率的解决方案。

Method: 1. 利用语义和不确定性先验在视图变换中抑制自由空间投影，采用显式无符号距离编码增强几何一致性，生成结构一致的稀疏3D表示；2. 设计级联稀疏补全模块，通过超交叉稀疏卷积和生成上采样实现高效粗到细推理；3. 提出基于对象上下文表示（OCR）的掩码解码器，从稀疏特征聚合全局语义上下文，通过轻量级查询-上下文交互细化体素预测，避免对体积特征的昂贵注意力操作。

Result: 在SemanticKITTI基准测试上的广泛实验表明，该方法在准确率上提升了7.34%，在效率上提升了57.8%，优于基线方法。

Conclusion: SUG-Occ通过利用3D场景固有稀疏性，结合语义和不确定性引导的稀疏学习策略，成功实现了在保持高精度的同时显著提升计算效率的3D语义占据预测，为自动驾驶实时部署提供了可行方案。
...
TL;DR: WetSAM：基于SAM的框架，通过双分支设计和卫星时间序列整合，从稀疏点监督实现湿地制图，显著提升性能至平均F1分数85.58%


<details>
  <summary>Details</summary>
Motivation: 湿地制图需要密集像素级标注但成本高昂，实际应用依赖稀疏点标签；现有深度学习模型在稀疏监督下性能差，湿地强烈的季节性和年际动态使单时相影像不足，导致显著制图误差；基础模型如SAM虽能从点提示泛化，但为静态图像设计，无法建模时序信息，在异质湿地中产生碎片化掩码

Method: 提出WetSAM框架：1) 时序提示分支通过分层适配器和动态时序聚合扩展SAM，从物候变异中解耦湿地特征；2) 空间分支使用时序约束的区域生长策略生成可靠密集伪标签；3) 双向一致性正则化联合优化两个分支
...
TL;DR: Map2Thought是一个用于3D视觉语言模型的可解释空间推理框架，通过度量认知地图和认知思维链实现显式几何推理，在有限监督下达到接近全监督的性能。
...
Method: 提出Map2Thought框架，包含两个核心组件：1) 度量认知地图：结合离散网格进行关系推理和连续度量尺度表示进行精确几何理解；2) 认知思维链：通过向量运算、边界框距离和遮挡感知外观顺序等确定性操作进行显式几何推理，生成基于3D结构的可解释推理轨迹。

Result: 实验表明，Map2Thought仅使用一半监督数据就达到59.9%的准确率，接近全监督基线60.9%。在VSI-Bench上，使用10%、25%、50%训练子集时，分别比最先进方法高出5.3%、4.8%和4.0%。

Conclusion: Map2Thought通过显式和可解释的空间推理机制，显著提升了3D视觉语言模型在有限监督下的性能，为3D理解提供了可解释的推理框架。
...
TL;DR: 提出MHA2MLA-VLM框架，将现有多头注意力VLM高效转换为多头潜在注意力架构，以压缩KV缓存并加速推理，同时保持性能
...
Method: 提出MHA2MLA-VLM框架，包含两个核心技术：1) 模态自适应部分RoPE策略，通过选择性掩码非必要维度支持传统和多模态设置；2) 模态解耦低秩近似方法，独立压缩视觉和文本KV空间。结合参数高效微调，最小化输出激活误差而非参数距离

Result: 在三个代表性VLM上的实验表明，MHA2MLA-VLM能以最小监督数据恢复原始模型性能，显著减少KV缓存占用，并能与KV量化无缝集成

Conclusion: MHA2MLA-VLM为现有VLM提供了一种参数高效、多模态感知的MLA转换方案，有效解决了KV缓存瓶颈问题，为大规模VLM的高效推理提供了实用解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型主要依赖稀疏轨迹标注的模仿学习，未能充分利用其作为生成模型的潜力，需要更强大的语言基础场景生成和长期规划能力

Method: 1) 训练VLA模型将自车和智能体动态编码为潜在token，监督来自规划、运动和语言任务；2) 语言条件自回归生成，给定多视角图像、场景描述和自车动作问题，生成未来潜在token和文本响应；3) 使用展开一致性损失稳定预测，减轻漂移并保持文本-动作对齐

Result: 在Bench2Drive上，GeRo将驾驶分数和成功率分别提升+15.7和+26.2；通过集成强化学习和生成展开，在闭环和开环性能上达到最先进水平，展示强大的零样本鲁棒性

Conclusion: GeRo展示了生成性、语言条件推理作为更安全、更可解释的端到端自动驾驶基础的潜力，支持时间一致的、语言基础的展开和长期推理
...
TL;DR: 提出ReScene4D方法解决稀疏时间采样的4D室内语义实例分割任务，通过跨观测信息共享实现时间一致的实例跟踪，并在3RScan数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 室内环境随时间动态变化（物体移动、出现、消失），需要从间歇性采集的3D扫描中保持时间一致的实例身份识别。现有3DSIS方法缺乏时间推理需要离散匹配，4D LiDAR方法依赖高频时间测量不适用于室内环境的长期演化。

Method: 提出ReScene4D方法，将3DSIS架构适配到4DSIS任务，无需密集观测。通过跨观测信息共享策略，利用共享上下文实现一致的实例跟踪，同时提升标准3DSIS质量。

Result: 定义了新指标t-mAP（扩展mAP以奖励时间身份一致性），在3RScan数据集上达到最先进性能，为理解演化室内场景建立了新基准。

Conclusion: ReScene4D成功解决了稀疏时间采样的4D室内语义实例分割任务，通过信息共享策略实现了时间一致的实例跟踪，并提升了分割质量，为动态室内场景理解提供了有效解决方案。
...
TL;DR: ShapeR：从随意拍摄的图像序列生成3D物体形状的新方法，通过多模态条件输入和整流流变换器，在真实场景中显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有3D形状生成方法依赖干净、无遮挡、良好分割的输入，这在真实场景中很少见。需要一种能够从随意拍摄序列中生成3D物体形状的方法

Method: 使用视觉惯性SLAM、3D检测算法和视觉语言模型提取稀疏SLAM点、多视角图像和机器生成描述。训练整流流变换器以这些模态为条件生成高保真度量3D形状。采用组合增强、课程训练和处理背景杂波等技术确保鲁棒性

Result: 在包含178个真实世界物体的新评估基准上，ShapeR显著优于现有方法，Chamfer距离比现有最佳方法提升2.7倍

Conclusion: ShapeR能够从随意拍摄的图像序列中生成高质量的3D物体形状，解决了现有方法在真实场景中的局限性，为实际应用提供了有效解决方案
...
TL;DR: UniX是一个统一的医学基础模型，通过解耦自回归理解分支和扩散生成分支，结合跨模态自注意力机制，在胸部X光图像理解和生成任务上实现协同性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有医学基础模型难以同时统一视觉理解和生成任务，因为这两个任务存在本质冲突：语义抽象与像素级重建。基于参数共享的自回归架构通常会导致一个或两个任务的性能受损。

Method: 1. 将任务解耦为自回归分支（理解）和扩散分支（高保真生成）；2. 引入跨模态自注意力机制，用理解特征动态指导生成过程；3. 采用严格的数据清洗流程和多阶段训练策略；4. 利用扩散模型优势实现高质量生成。

Result: 在两个代表性基准测试中，UniX仅使用LLM-CXR四分之一参数的情况下，理解性能（Micro-F1）提升46.1%，生成质量（FD-RadDino）提升24.2%。性能与任务专用模型相当。

Conclusion: UniX通过解耦架构和跨模态协同机制，实现了医学图像理解和生成的协同统一，为协同医学图像理解和生成建立了可扩展的范式。
...
### [35] [Analytic Bijections for Smooth and Interpretable Normalizing Flows](https://arxiv.org/abs/2601.10774)
*Mathis Gerdes,Miranda C. N. Cheng*

Main category: cs.LG

TL;DR: 提出三种全局平滑、定义在全实数域且具有解析逆的标量双射函数族（三次有理、sinh、三次多项式），以及径向流架构，在保持训练稳定性的同时显著减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 现有归一化流设计面临权衡：仿射变换平滑且可解析求逆但表达能力有限；单调样条具有局部控制但仅分段平滑且定义在有界域；残差流平滑但需要数值求逆。需要结合各种方法优点的解决方案。

Method: 引入三种解析双射函数族：三次有理、sinh和三次多项式，它们全局平滑（C^∞）、定义在全实数域且具有闭式解析逆。此外提出径向流架构，通过直接参数化径向坐标变换同时保持角度方向不变。

Result: 在耦合流中作为即插即用替换，匹配或超越样条性能。径向流表现出卓越的训练稳定性，产生几何可解释的变换，对于具有径向结构的目标，能以1000倍更少的参数实现与耦合流相当的质量。在φ^4晶格场理论的高维物理问题中，新双射函数优于仿射基线并解决模式崩溃问题。

Conclusion: 提出的三种解析双射函数族结合了现有方法的优点，而径向流架构为具有径向结构的问题提供了高效且可解释的解决方案，在保持性能的同时显著减少参数需求。
...
### [36] [Unified Optimization of Source Weights and Transfer Quantities in Multi-Source Transfer Learning: An Asymptotic Framework](https://arxiv.org/abs/2601.10779)
*Qingyue Zhang,Chang Chu,Haohao Fu,Tianren Peng,Yanru Wu,Guanbo Huang,Yang Li,Shao-Lun Huang*

Main category: cs.LG

TL;DR: UOWQ框架统一优化多源迁移学习中的源权重和迁移数量，证明适当调整权重后使用所有源样本总是最优的，并提供理论分析和实用算法。


<details>
  <summary>Details</summary>
Motivation: 传统多源迁移学习方法通常单独优化源权重或迁移样本数量，忽略了二者的联合考虑，且均匀迁移可能导致负迁移。需要理论框架来统一优化这两个关键因素。

Method: 提出UOWQ理论框架，基于KL散度泛化误差的渐近分析，将多源迁移学习建模为参数估计问题。证明权重调整后使用所有源样本最优，为单源场景提供闭式解，为多源场景开发基于凸优化的数值方法，并设计实用算法。

Result: 在DomainNet和Office-Home等真实基准测试中，UOWQ始终优于强基线方法，验证了理论预测和框架的实际有效性。

Conclusion: UOWQ框架通过联合优化源权重和迁移数量，解决了多源迁移学习中的负迁移问题，提供了理论保证和实用算法，显著提升了模型在数据稀缺场景下的性能。
...
TL;DR: 研究者整合了多个公开的1型糖尿病数据集，创建了名为MetaboNet的统一资源，包含3135名患者和1228患者年的连续血糖监测与胰岛素泵数据，提供公开和受限两种访问方式。
...
Method: 整合多个公开的1型糖尿病数据集，要求同时包含连续血糖监测数据和相应的胰岛素泵给药记录，保留碳水化合物摄入和体力活动等辅助信息。创建了包含3135名患者的MetaboNet数据集，提供完全公开子集和需数据使用协议限制的子集两种访问方式。

Result: MetaboNet数据集包含3135名患者和1228患者年的重叠CGM和胰岛素数据，规模远超现有独立基准数据集。数据集涵盖广泛的血糖谱和人口统计学特征，可通过https://metabo-net.org/立即下载公开子集，受限子集需通过相应申请流程访问。

Conclusion: 提出了一个用于1型糖尿病研究的统一公共数据集，描述了其无限制和受数据使用协议管理组件的访问途径。该数据集能够产生比单个数据集更具泛化性的算法性能。
...
### [38] [Mugi: Value Level Parallelism For Efficient LLMs](https://arxiv.org/abs/2601.10823)
*Daniel Price,Prabhu Vellaisamy,John Shen,Di Wu*

Main category: cs.LG

TL;DR: 该论文提出了一种名为Mugi的新型VLP架构，通过泛化VLP用于非线性近似、优化小批量GEMM以及支持完整LLM工作负载，显著提升了LLM的吞吐量、能效和可持续性。


<details>
  <summary>Details</summary>
Motivation: 现有VLP技术主要针对大批量、低精度的激活-权重GEMM操作，但LLM中存在更复杂的非线性操作和小批量场景，需要探索VLP如何更好地应用于LLM以提升效率。

Method: 1. 将VLP泛化用于非线性近似，采用以值为中心的方法，为重要值分配更高精度；2. 优化VLP用于小批量GEMM和非对称输入，结合权重量化、KV缓存量化和分组查询注意力等LLM优化技术；3. 设计新的VLP架构Mugi，集成上述创新并支持完整LLM工作负载。

Result: Mugi在非线性softmax操作上实现高达45倍吞吐量提升和668倍能效提升，在LLM上实现2.07倍吞吐量提升和3.11倍能效提升，同时将LLM运行碳足迹降低1.45倍，硬件隐含碳降低1.48倍。

Conclusion: 通过泛化VLP用于非线性近似、优化小批量GEMM以及设计Mugi架构，成功将VLP技术应用于LLM，显著提升了性能、能效和可持续性，为高效LLM推理提供了新的硬件架构解决方案。
...
### [39] [AI-Guided Human-In-the-Loop Inverse Design of High Performance Engineering Structures](https://arxiv.org/abs/2601.10859)
...
Method: 采用U-Net架构的图像分割模型作为AI副驾驶，在合成数据集上训练预测用户偏好区域（最长拓扑构件或最复杂结构连接），将预测结果作为AI推荐呈现给用户。

Result: 模型成功预测合理的修改区域，在多样化和非标准拓扑优化问题上表现出泛化能力，并展现出超出单区域选择训练数据的涌现行为。集成AI副驾驶的方法可将线性屈曲载荷提高39%，总设计时间仅增加15秒。

Conclusion: AI副驾驶的集成显著提升了人机协同拓扑优化的效率和性能，在保持设计时间基本不变的情况下，实现了结构性能的显著改善。
...
### [40] [Beyond Accuracy: A Stability-Aware Metric for Multi-Horizon Forecasting](https://arxiv.org/abs/2601.10863)
*Chutian Ma,Grigorii Pomazkin,Giacinto Paolo Saggese,Paul Smith*

Main category: cs.LG

TL;DR: 提出了一种新的概率多步预测评估指标——预测准确性与一致性评分（forecast AC score），该指标同时考虑多步预测的准确性和时间一致性，并通过可微目标函数优化季节性ARIMA模型，在M4 Hourly数据集上显著降低了预测波动性。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列预测方法仅优化准确性，忽略了时间一致性（即模型在不同预测起点对同一未来事件预测的一致性）。这种单一目标无法全面评估概率多步预测的质量，需要同时考虑准确性和稳定性的综合评估指标。

Method: 提出了forecast AC score评估指标，该指标可同时衡量多步预测的准确性和时间一致性，并允许用户指定权重来平衡这两个需求。将该评分实现为可微目标函数，用于训练季节性ARIMA模型，并在M4 Hourly基准数据集上进行评估。

Result: 与传统最大似然估计相比，AC优化模型在M4 Hourly数据集上取得了显著改进：对相同目标时间戳实现了75%的预测波动性降低，同时保持相当或改进的点预测准确性。

Conclusion: forecast AC score为概率多步预测提供了更全面的质量评估框架，通过同时优化准确性和时间一致性，显著提高了预测的稳定性，为时间序列预测提供了新的优化方向。
...
### [41] [Action Shapley: A Training Data Selection Metric for World Model in Reinforcement Learning](https://arxiv.org/abs/2601.10905)
...
<details>
  <summary>Details</summary>
Motivation: 在强化学习中，世界模型的质量对系统效能和可解释性至关重要，而模型质量又高度依赖训练数据。传统训练数据选择方法存在主观性和偏差问题，需要一种公平、客观的数据选择度量方法。

Method: 提出Action Shapley作为与模型无关的训练数据选择度量，基于合作博弈理论中的Shapley值概念。为应对传统Shapley值计算的指数复杂度，设计了随机动态算法来高效计算Action Shapley值。

Result: 在五个数据受限的实际案例研究中，所提算法相比传统指数时间计算实现了超过80%的计算效率提升。基于Action Shapley的训练数据选择策略在性能上持续优于临时性数据选择方法。

Conclusion: Action Shapley为世界模型训练提供了公平、高效的数据选择框架，其随机动态算法有效解决了计算复杂度问题，在实际应用中展现出显著优势。
...
Method: 提出FAConvLSTM作为ConvLSTM2D的即插即用替代方案：1) 使用轻量级[1×1]瓶颈和共享深度空间混合因子化循环门计算；2) 多尺度扩张深度分支和挤压-激励重校准实现跨空间尺度的高效建模；3) 窥视孔连接增强时间精度；4) 轻量级轴向空间注意力机制稀疏应用于时间维度以捕获遥相关依赖；5) 专用子空间头通过带固定季节性位置编码的时间自注意力生成紧凑的每时间步嵌入。

Result: 在多元时空气候数据上的实验表明，FAConvLSTM比标准ConvLSTM产生更稳定、可解释和鲁棒的潜在表示，同时显著降低计算开销。

Conclusion: FAConvLSTM通过因子化注意力机制有效解决了ConvLSTM在高分辨率地球观测数据建模中的局限性，在效率、空间表达能力和物理可解释性方面实现了同步改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有分割学习系统主要依赖一阶优化，需要客户端存储激活值等中间量用于反向传播，导致内存开销大，削弱了模型分割的优势。而零阶优化虽能减少内存使用，但收敛慢且性能下降。需要解决内存效率与优化效果之间的根本权衡。

Method: HOSL采用混合阶优化策略：客户端使用内存高效的零阶梯度估计，消除反向传播和激活存储；服务器端使用一阶优化确保快速收敛和竞争性能。理论上证明收敛率取决于客户端模型维度而非完整模型维度。

Result: 在OPT模型（125M和1.3B参数）的6个任务上实验表明：HOSL相比一阶方法减少客户端GPU内存达3.7倍，精度仅降低0.20%-4.23%；相比零阶基线提升性能达15.55%。

Conclusion: HOSL通过客户端零阶优化和服务器端一阶优化的混合策略，有效解决了分割学习中内存效率与优化效果的权衡问题，为边缘设备上的内存高效训练提供了有效方案。
...
### [44] [Transient learning dynamics drive escape from sharp valleys in Stochastic Gradient Descent](https://arxiv.org/abs/2601.10962)
*Ning Yang,Yikuan Zhang,Qi Ouyang,Chao Tang,Yuhai Tu*

Main category: cs.LG

TL;DR: SGD通过非平衡机制选择平坦解：噪声重塑损失景观为有效势能，偏好平坦区域；瞬态探索阶段后，能量势垒增长导致"冻结"机制，将动态困在单个盆地中。


<details>
  <summary>Details</summary>
Motivation: 理解SGD为何偏好平坦、更具泛化性的解，揭示其动力学起源和非平衡机制，为优化算法设计提供理论基础。

Method: 分析SGD学习动力学，进行数值实验观察轨迹行为，使用可处理的物理模型研究噪声如何重塑损失景观，识别瞬态冻结机制。

Result: SGD噪声将损失景观重塑为有效势能，偏好平坦解；存在瞬态探索阶段，轨迹逃离尖锐谷并转向平坦区域；随着训练进行，能量势垒增长导致"冻结"机制，将动态困在单个盆地中；增加噪声强度延迟冻结，增强向平坦最小值的收敛。

Conclusion: 研究提供了统一的物理框架，连接学习动力学、损失景观几何和泛化能力，为设计更有效的优化算法提供了原则指导。
...
### [45] [Toward Adaptive Grid Resilience: A Gradient-Free Meta-RL Framework for Critical Load Restoration](https://arxiv.org/abs/2601.10973)
*Zain ul Abdeen,Waris Gill,Ming Jin*

Main category: cs.LG

TL;DR: 提出MGF-RL框架，结合元学习和进化策略，实现配电网故障恢复策略的快速自适应，减少对新场景的重新训练需求


<details>
  <summary>Details</summary>
Motivation: 极端事件后恢复关键负荷需要自适应控制，但可再生能源不确定性、可调度资源有限和非线性动态使恢复困难；标准RL泛化能力差，对新故障配置需要大量重新训练

Method: 提出元引导的无梯度RL框架，结合一阶元学习和进化策略，从历史故障经验学习可迁移初始化，无需梯度计算即可适应非线性约束系统动态

Result: 在IEEE 13总线和123总线测试系统中，MGF-RL在可靠性、恢复速度和自适应效率方面优于标准RL、MAML元RL和模型预测控制，对未见故障和可再生能源模式泛化能力强

Conclusion: MGF-RL为可再生能源丰富的配电网实时负荷恢复提供有效解决方案，具有理论保证的次线性遗憾界，支持实际应用
...
### [46] [Reasoning Distillation for Lightweight Automated Program Repair](https://arxiv.org/abs/2601.10987)
*Aanand Balasubramanian,Sashank Silwal*

Main category: cs.LG

TL;DR: 轻量级符号推理监督能提升紧凑型自动程序修复模型的修复类型分类性能，通过推理蒸馏方法让大模型提供结构化符号推理标签，在保持模型大小不变的情况下改善性能，特别是在低频错误类别上。


<details>
  <summary>Details</summary>
Motivation: 小型代码模型在资源受限环境中具有吸引力，但它们通常只产生单一预测，不清楚它们是否学习了有意义的程序结构还是仅依赖浅层相关性。需要验证轻量级符号推理监督是否能提升修复类型分类性能。
...
Conclusion: 符号推理蒸馏是提升轻量级程序修复模型可解释性和鲁棒性的实用方法，表明结构化推理监督能帮助小型模型学习更有意义的程序结构。
...
### [47] [Constant Metric Scaling in Riemannian Computation](https://arxiv.org/abs/2601.10992)
*Kisung You*

Main category: cs.LG

TL;DR: 本文澄清了黎曼度量常数缩放对几何量和优化算法的影响，区分了哪些量会变化（如范数、距离、体积），哪些几何结构保持不变（如联络、测地线、指数映射）。


<details>
  <summary>Details</summary>
Motivation: 在计算应用中，黎曼度量的常数缩放经常出现，但这一操作的影响在实践中并不总是清晰，容易与曲率变化、流形结构改变或坐标表示混淆。本文旨在澄清常数度量缩放的影响，区分变化与不变的几何量。

Method: 本文采用理论分析方法，系统地考察黎曼度量常数缩放对各种几何量和结构的影响。通过数学推导和论证，明确区分哪些量会随缩放变化，哪些几何结构保持不变量。

Result: 分析表明：范数、距离、体积元素、梯度大小等量会随度量缩放而变化；而Levi-Civita联络、测地线、指数映射、对数映射、平行移动等几何结构保持不变。在黎曼优化中，常数度量缩放可解释为步长的全局重缩放，而非底层几何的修改。

Conclusion: 常数度量缩放不会改变黎曼流形的几何结构，只是对某些量的重新标度。在黎曼计算中，全局度量尺度参数可以引入而不改变方法所依赖的几何结构，这有助于澄清优化算法中的缩放效应。
...
### [48] [Backdoor Attacks on Multi-modal Contrastive Learning](https://arxiv.org/abs/2601.11006)
*Simi D Kuniyilh,Rita Machacy*

Main category: cs.LG

TL;DR: 该论文对对比学习中的后门攻击进行了全面比较性综述，分析了威胁模型、攻击方法、目标领域和现有防御措施，强调了对比学习特有的安全漏洞，并讨论了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 对比学习已成为跨领域自监督表示学习的主流方法，但最近研究表明其易受后门和数据中毒攻击。攻击者可通过操纵预训练数据或模型更新插入隐藏恶意行为，这对工业和分布式环境中的系统安全部署构成重大威胁。

Method: 采用系统性文献综述方法，对对比学习中的后门攻击进行综合分析。研究内容包括：威胁模型分类、攻击方法比较、目标领域分析（视觉、多模态、图、联邦学习等）、现有防御措施评估，以及对比学习特有漏洞的识别。

Result: 研究发现对比学习存在特定的安全漏洞，攻击者可通过数据污染和模型更新操纵实现后门植入。论文总结了该领域的最新进展，揭示了不同攻击方法的有效性，并评估了现有防御措施的局限性。

Conclusion: 对比学习中的后门攻击是一个严重的安全威胁，需要更强大的防御机制。论文强调了对比学习特有的脆弱性，提出了未来研究方向，包括开发更有效的检测和缓解技术，以确保在工业和分布式环境中的安全部署。
...
### [49] [Combating Spurious Correlations in Graph Interpretability via Self-Reflection](https://arxiv.org/abs/2601.11021)
*Kecheng Cai,Chenyang Xu,Chao Peng*

Main category: cs.LG

TL;DR: 该论文提出了一种自反思框架，用于提升图学习模型在具有强伪相关性的Spurious-Motif基准数据集上的可解释性性能。


<details>
  <summary>Details</summary>
Motivation: 现有可解释图学习方法在Spurious-Motif基准数据集上表现显著较差，该数据集故意设计包含伪相关性，使得模型难以区分真正相关结构与误导性模式。需要提升模型在这种具有挑战性数据集上的可解释性。

Method: 提出自反思框架，可与现有可解释图学习方法集成。当基础方法生成节点和边的重要性分数后，该框架将这些预测反馈给原始方法进行第二轮评估，形成迭代过程。同时从图表示学习角度分析改进原因，提出基于此反馈机制的微调训练方法。

Result: 自反思技术（通常用于大语言模型处理复杂任务）可以有效适应并增强在具有强伪相关性数据集上的可解释性。该框架能够提升现有方法在Spurious-Motif基准上的性能。

Conclusion: 自反思框架为提升可解释图学习方法在具有挑战性的伪相关数据集上的性能提供了有效途径，将大语言模型中的自反思概念成功迁移到图学习领域。
...
### [50] [AVP-Pro: An Adaptive Multi-Modal Fusion and Contrastive Learning Approach for Comprehensive Two-Stage Antiviral Peptide Identification](https://arxiv.org/abs/2601.11028)
...
<details>
  <summary>Details</summary>
Motivation: 现有抗病毒肽识别方法在捕捉复杂序列依赖性和区分高相似度样本方面存在局限性，需要开发更准确、更具判别力的预测工具。

Method: 提出AVP-Pro两阶段框架：1）构建包含10种描述符的全景特征空间，采用分层融合架构整合自注意力和自适应门控机制，动态调节CNN提取的局部基序和BiLSTM捕获的全局依赖权重；2）采用基于BLOSUM62增强的OHEM对比学习策略，锐化模型判别边界。
...
### [51] [OpFML: Pipeline for ML-based Operational Forecasting](https://arxiv.org/abs/2601.11046)
...
<details>
  <summary>Details</summary>
Motivation: 机器学习在气候和地球科学中应用广泛，传统火灾风险评估方法常高估风险，需要更有效的操作预测系统

Method: 开发OpFML：一个可配置、可适应的机器学习管道，用于周期性预测，并以每日火灾危险指数预测为例展示其功能

Result: 提出了OpFML代码框架，展示了其在火灾危险指数预测中的应用能力，并概述了其各种特性

Conclusion: OpFML为基于机器学习的操作预测系统提供了一个灵活、可配置的解决方案，特别适用于火灾危险评估等周期性预测任务
...
TL;DR: 提出Soft-BCT模型，一种用于实值时间序列的新型贝叶斯上下文树模型，采用软（概率性）上下文空间分割而非硬分割，基于变分推断进行学习，在真实数据集上表现与先前BCT相当或更优。


<details>
  <summary>Details</summary>
Motivation: 先前用于实值时间序列的BCT模型采用硬（确定性）上下文空间分割，这可能限制了模型的灵活性和表达能力。需要一种能够进行软（概率性）分割的BCT模型来更好地捕捉时间序列的复杂依赖结构。

Method: 提出Soft-BCT模型，采用概率性上下文空间分割而非确定性分割。基于变分推断开发学习算法，通过概率框架处理上下文空间的不确定性，实现更灵活的模型结构学习。

Result: 在多个真实世界数据集上的实验表明，Soft-BCT模型表现出与先前BCT模型相当或更优的性能，验证了软分割方法的有效性。

Conclusion: Soft-BCT通过引入软上下文空间分割扩展了BCT模型框架，为实值时间序列建模提供了更灵活的方法，基于变分推断的学习算法有效实现了模型参数估计。
...
TL;DR: DP-SFT：一种两阶段子空间微调方法，通过将DP噪声仅注入到低维任务特定子空间中，在保持差分隐私保证的同时显著降低噪声幅度，提升大语言模型微调的性能和稳定性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在下游任务上的微调通常依赖敏感数据，存在隐私风险。差分隐私（DP）虽能提供严格的隐私保证，但在高维参数空间中注入噪声会产生大范数扰动，导致性能下降和训练不稳定。
...
Conclusion: DP-SFT通过将DP噪声限制在任务相关的低维子空间中，有效降低了噪声幅度，在保持正式DP保证的同时显著改善了隐私保护微调的性能和稳定性。
...
TL;DR: 提出一种基于大语言模型的约束生成方法，通过生成约束集而非传统成对约束来减少资源消耗，并设计专门的约束聚类算法处理可能不准确的约束，在保持聚类精度的同时将LLM查询次数减少20倍以上。


<details>
  <summary>Details</summary>
Motivation: 传统聚类方法通过人工提供must-link和cannot-link约束来提升聚类精度，但这种方法成本高且难以扩展。随着大语言模型的发展，研究者开始探索基于LLM的自动约束生成，但现有方法在查询效率和约束准确性方面仍有改进空间。

Method: 提出新颖的约束生成方法，生成约束集而非成对约束以减少资源消耗；设计专门的约束聚类算法，包含置信度阈值和惩罚机制来处理可能不准确的LLM生成约束。

Result: 在五个文本数据集上的评估显示，该方法在保持与最先进算法相当的聚类精度的同时，将LLM查询次数减少了20倍以上，显著降低了约束生成成本。

Conclusion: 该方法通过高效的约束生成策略和鲁棒的约束聚类算法，在减少资源消耗的同时保持了聚类质量，为基于LLM的约束聚类提供了实用且高效的解决方案。
...
TL;DR: 提出S2NO神经算子，结合谱方法和空间卷积，实现复杂几何体上高保真形变预测，结合进化算法优化材料分布设计
...
Conclusion: S2NO显著提高了复杂形状形变编程的效率和能力，为先进应用如共形植入物部署和气动形变提供了有效工具
...
<details>
  <summary>Details</summary>
Motivation: 金融信用风险模型面临两个关键问题：1）传统二元分类忽略违约时间，将早期违约者（高损失）与晚期违约者（低损失）等同处理；2）数据保护法规（如GDPR、CCPA）禁止跨境共享借款人数据，而跨机构学习对模型有益。

Method: 提出了联邦生存学习框架FSL-BDP，结合贝叶斯差分隐私（BDP），在不集中敏感数据的情况下建模违约时间轨迹。该框架提供贝叶斯（数据依赖）差分隐私保证，同时允许机构联合学习风险动态。

Result: 在三个真实信用数据集（LendingClub、SBA、Bondora）上的实验表明：1）联邦学习从根本上改变了隐私机制相对有效性；2）在集中式设置中经典DP优于贝叶斯DP，但在联邦学习中贝叶斯DP获益更大（+7.0% vs +1.4%）；3）贝叶斯DP达到接近非私有性能水平，在多数参与客户端中优于经典DP。
...
TL;DR: CaMol：基于因果推理的上下文感知图学习框架，用于小样本分子性质预测，通过发现因果亚结构提高准确性和可解释性
...
Method: 提出CaMol框架：1）构建编码化学知识（官能团、分子、性质关系）的上下文图指导因果亚结构发现；2）提出可学习的原子掩码策略分离因果亚结构与混杂亚结构；3）引入分布干预器，通过后门调整结合因果亚结构与化学基础的混杂因子，从真实化学变异中解耦因果效应。

Result: 在多个分子数据集上的实验表明，CaMol在小样本任务中实现了优越的准确性和样本效率，展示了对未见性质的泛化能力。发现的因果亚结构与官能团的化学知识高度一致，支持模型可解释性。

Conclusion: CaMol通过因果推理视角解决了小样本分子性质预测的关键挑战，不仅提高了预测性能，还提供了与化学知识一致的因果亚结构解释，为分子性质预测提供了更可靠和可解释的框架。
...
Method: 比较两种方法：1) 有监督分类管道，使用正常和故障行为的标记数据；2) 基于自编码器的无监督异常检测，仅使用健康发动机数据学习正常操作模型，将偏差标记为潜在故障
...
TL;DR: 该论文探讨聚类算法中抽象与表示之间的平衡，分析K-means、子空间聚类和深度聚类等方法如何实现不同权衡，并展望未来自适应平衡的研究方向。


<details>
  <summary>Details</summary>
Motivation: 聚类需要在抽象（忽略个体细节）和表示（强调群体关键特征）之间取得平衡。现有算法在这两个目标之间存在冲突，需要开发能更好平衡二者的方法。

Method: 分析不同聚类方法的抽象-表示权衡：K-means采用高抽象度（细节平均化）和简单表示（原始空间高斯分布）；子空间聚类通过分离相关信息和无关信息的潜在空间；深度聚类使用基于质心和密度的聚类损失来强制抽象。

Result: 随着表示表达能力的增强，需要在目标函数中显式强制抽象以确保进行聚类而非单纯表示学习。子空间聚类方法通过分离两个潜在空间有效平衡抽象与表示。

Conclusion: 未来聚类研究需要更自适应地平衡抽象与表示以提高性能、能效和可解释性。人脑在聚类和单次学习等任务中能自动找到最佳平衡点，这为算法改进提供了重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 现有源自由通用域自适应方法仅处理单一源域到目标域的转移，但在实际应用中，模型需要连续适应多个不同的未标记目标域，因此需要研究持续SF-UniDA问题

Method: 结合高斯混合模型伪标签和均值教师框架，在均值教师架构中集成GMM伪标签机制以提高长期适应稳定性，并引入一致性损失增强鲁棒性

Result: GMM-COMET在所有评估场景中持续改进源模型性能，成为持续SF-UniDA领域的首个强基线方法

Conclusion: 该方法成功解决了持续源自由通用域自适应问题，为后续研究提供了有效基准，代码已开源
...
TL;DR: 提出一种结构解耦的多尺度时间序列生成框架，通过双路径VQ-VAE分离趋势和季节成分，采用粗到细的自回归生成方式，在减少参数量的同时提升生成质量。
...
Method: 1. 将序列编码为多个时间分辨率的离散标记，采用粗到细的自回归生成方式，保持层次依赖关系；2. 引入双路径VQ-VAE解耦趋势和季节成分，学习语义一致的潜在表示；3. 提出基于引导的重建策略，利用粗粒度季节信号作为先验指导细粒度季节模式的重建。

Result: 在六个数据集上的实验表明，该方法比现有方法产生更高质量的时间序列。模型在显著减少参数数量的情况下实现了强大性能，并在生成长期序列方面表现出卓越能力。

Conclusion: 提出的结构解耦多尺度生成框架有效解决了时间序列的结构复杂性，通过成分解耦和多尺度生成策略，在保持参数效率的同时显著提升了生成质量。
...
TL;DR: FAQ提出了一种基于家族先验知识的校准数据再生框架，通过利用同家族大模型生成高质量校准样本，显著提升后训练量化的精度。
...
Method: FAQ框架首先将原始校准样本输入同家族的大模型中，利用高度一致的知识体系再生高保真校准数据；然后通过专家指导下的组竞争选择最佳样本，并进行重归一化处理以增强标准PTQ效果。

Result: 在包括Qwen3-8B在内的多个模型系列上的实验表明，FAQ相比使用原始校准数据的基线方法，将精度损失降低了最高28.5%。

Conclusion: FAQ通过利用同家族LLM的先验知识生成高质量校准数据，有效解决了传统PTQ中校准数据代表性不足的问题，展示了强大的应用潜力和贡献。
...
TL;DR: SDFLoRA：一种选择性双模块联邦LoRA方法，通过分解全局模块和本地模块解决联邦学习中LoRA的秩异构问题，实现更好的隐私-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 联邦学习用于大语言模型时，不同客户端可能采用不同的低秩配置（秩异构），导致直接聚合LoRA更新存在偏差和不稳定。现有方法强制统一秩或将异构更新对齐到共享子空间，这会过度约束客户端特定语义、限制个性化，并在差分隐私噪声下提供较弱的本地信息保护。
...
Conclusion: SDFLoRA通过分解全局和本地模块有效解决了联邦学习中LoRA的秩异构问题，在保持个性化能力的同时实现了更好的隐私保护和模型性能。
...
TL;DR: 提出MAD-BNO框架，仅使用边界数据学习偏微分方程算子，通过数学人工数据方法生成训练数据，无需全域采样或数值模拟
...
Method: 结合数学人工数据(MAD)方法，从目标问题的基本解直接合成训练数据，学习边界到边界的映射。训练后通过边界积分公式恢复内部任意位置的解，支持多种边界条件和源项

Result: 在二维Laplace、Poisson和Helmholtz方程的基准测试中，MAD-BNO达到或优于现有神经算子方法的精度，同时显著减少训练时间。框架可扩展到三维问题和复杂几何

Conclusion: MAD-BNO为线性偏微分方程提供了一种高效的数据驱动算子学习方法，仅需边界数据，无需全域采样或数值模拟，具有良好扩展性
...
TL;DR: PaST框架通过提取领域无关的技能向量，在轻量级SFT后线性注入知识操作技能，解决LLM知识更新中SFT更新事实但RL成本高的问题，实现高效知识适应。


<details>
  <summary>Details</summary>
Motivation: LLM面临"知识截止"挑战，其冻结的参数化记忆无法直接内化新信息。SFT常用于更新模型知识，但通常只更新事实内容而无法可靠提升模型使用新信息进行问答或决策的能力。RL对于获取推理技能至关重要，但其高计算成本使得在线适应不切实际。作者观察到SFT和RL诱导的参数更新几乎正交，这为模块化技能转移提供了机会。

Method: 提出参数化技能转移（PaST）框架，支持模块化技能转移以实现高效有效的知识适应。方法包括：1）从源域提取领域无关的技能向量；2）在目标模型经过轻量级SFT学习新数据后，线性注入知识操作技能。该方法利用SFT和RL参数更新的正交性，将RL获得的推理技能与SFT获得的事实知识解耦。

Result: 在知识整合QA（SQuAD、LooGLE）和智能体工具使用基准（ToolBench）上验证了方法的有效性。在SQuAD上，PaST比最先进的自我编辑SFT基线高出9.9分。在LooGLE的长上下文QA中，PaST获得8.0分的绝对准确率提升。在ToolBench上，零样本成功率平均提高+10.3分，跨工具类别一致提升，表明技能向量具有良好的可扩展性和跨域可转移性。

Conclusion: PaST框架通过解耦知识获取和技能学习，实现了高效的知识适应。该方法不仅显著提升了知识整合QA的性能，还展示了在长上下文和工具使用任务中的强大可扩展性和跨域转移能力，为解决LLM知识更新问题提供了新的有效途径。
...
TL;DR: 首次提出具有接近最优样本复杂度的不可知增强算法，在固定其他参数时运行时间为多项式


<details>
  <summary>Details</summary>
Motivation: 增强方法在经典设置中已有深入理解，但在不可知设置中研究较少。现有不可知增强算法要么样本复杂度不理想，要么运行时间指数级。需要开发同时具有接近最优样本复杂度和多项式运行时间的算法。

Method: 提出新的不可知增强算法，在固定其他参数时，运行时间相对于样本大小为多项式复杂度。该算法将弱学习器转化为强学习器，不依赖数据分布假设。

Result: 首次实现了具有接近最优样本复杂度的不可知增强算法，且运行时间为多项式。解决了arXiv:2503.09384中指数时间算法的问题，填补了理论空白。

Conclusion: 该工作为不可知增强提供了首个实用的高效算法，平衡了样本复杂度和计算效率，推动了增强理论在无分布假设场景中的应用。
...
TL;DR: 本研究通过整合尿液代谢组学和可解释机器学习框架，识别与ADHD相关的生物化学特征，开发了一种基于生物学的客观诊断方法。


<details>
  <summary>Details</summary>
Motivation: 注意力缺陷多动障碍（ADHD）是一种普遍存在的神经发育障碍，目前缺乏客观的诊断工具，迫切需要建立基于生物学的客观诊断框架，以推动精准精神病学的发展。

Method: 研究采用靶向代谢组学分析52名ADHD患者和46名对照参与者的尿液样本，使用具有嵌入式特征选择功能的Closest Resemblance（CR）分类器进行数据分析，并与随机森林和K最近邻分类器进行比较。

Result: CR模型在基于14种代谢物的简化面板上表现最佳，AUC > 0.97。这些代谢物包括多巴胺4-硫酸盐、N-乙酰天冬氨酰谷氨酸和瓜氨酸等，映射到多巴胺能神经传递和氨基酸代谢途径，为ADHD病理生理学提供了机制性见解。

Conclusion: 该研究展示了一个结合代谢组学和可解释机器学习的转化框架，CR分类器的透明决策边界和低计算成本支持其整合到靶向代谢组学检测和未来即时诊断平台中，为ADHD的客观、生物学信息诊断策略提供了新途径。
...
TL;DR: FORESTLLM：结合决策森林结构偏置与LLM语义推理的少样本表格数据学习框架，仅训练时使用LLM设计轻量可解释森林模型，测试时无需LLM推理
...
Method: 1. 语义分割准则：LLM基于标注和未标注数据评估候选分割的语义连贯性，构建更鲁棒可泛化的树结构；2. 一次性上下文推理机制：LLM将决策路径及支撑样本提炼为确定性预测，用语义信息输出替代噪声经验估计

Result: 在多样化少样本分类和回归基准测试中，FORESTLLM实现了最先进的性能表现

Conclusion: FORESTLLM统一了决策森林的结构归纳偏置与LLM的语义推理能力，仅训练时使用LLM作为离线模型设计器，将丰富上下文知识编码到轻量可解释森林模型中，测试时无需LLM推理，解决了少样本表格数据学习的关键挑战
...
TL;DR: SPREAD框架解决扩散语言模型在检索增强生成中的语义漂移问题，通过查询相关性引导的去噪策略提高生成精度


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在自然语言处理任务中表现出色，但其在检索增强生成框架中的潜力尚未充分探索。现有DLMs在RAG中存在生成精度有限的问题，主要原因是响应语义漂移，即生成答案逐渐偏离查询的原始语义。

Method: 提出SPREAD（Semantic-Preserving REtrieval-Augmented Diffusion）框架，引入查询相关性引导的去噪策略。该方法主动引导去噪轨迹，确保生成过程始终锚定查询语义，有效抑制语义漂移。

Result: 实验结果表明，SPREAD显著提高了RAG框架中生成答案的精度，有效缓解了响应语义漂移问题。

Conclusion: SPREAD框架成功解决了扩散语言模型在检索增强生成中的语义漂移问题，通过查询引导的去噪策略实现了更精确的语义保持生成。
...
TL;DR: FEATHer是一种用于边缘设备上时间序列预测的超轻量级模型，通过多尺度频率分解和共享密集时间核实现高效长程预测，仅需400个参数即可在8个基准测试中获得60个第一名结果。


<details>
  <summary>Details</summary>
Motivation: 工业领域（如制造和智能工厂）需要边缘设备（PLC、微控制器）上的时间序列预测模型，这些设备有严格的延迟和内存限制（参数限制在几千个），传统深度架构在此不实用。

Method: 提出FEATHer模型：1）超轻量级多尺度频率分解为频率通路；2）共享密集时间核（投影-深度卷积-投影，无循环或注意力）；3）基于频谱特征自适应融合表示的分支门控；4）通过周期下采样捕获季节性的稀疏周期核。

Result: 在8个基准测试中取得最佳排名，获得60个第一名结果，平均排名2.05，模型参数最少仅400个，在严格约束下优于基线模型。

Conclusion: 可靠的长期预测在受限边缘硬件上是可行的，为工业实时推理提供了实用方向，证明了超轻量级架构在边缘时间序列预测中的有效性。
...
TL;DR: 提出基于配对自编码器的数据驱动潜在空间推理框架，通过连接参数空间和观测空间的自编码器潜在空间映射，处理观测不一致性并解决逆问题。
...
Method: 使用两个自编码器分别处理参数空间和观测空间，通过学习两个潜在空间之间的映射关系，构建正则化逆问题的替代模型，在低维信息丰富的潜在空间中进行优化。

Result: 相比单独使用配对自编码器和相同架构的端到端编码器-解码器，该方法在数据不一致场景下能产生更准确的重建结果，特别是在医学层析成像和地球物理地震波形反演中表现优异。

Conclusion: 该框架能灵活处理各种数据不一致性，同时保持物理一致性，可广泛应用于科学和工程领域的多种逆问题。
...
### [72] [When Are Two Scores Better Than One? Investigating Ensembles of Diffusion Models](https://arxiv.org/abs/2601.11444)
*Raphaël Razafindralambo,Rémy Sun,Frédéric Precioso,Damien Garreau,Pierre-Alexandre Mattei*

Main category: cs.LG

TL;DR: 扩散模型集成在理论上能改善分数匹配损失和似然，但在图像数据上未能一致提升感知质量指标如FID，而在表格数据上某些集成策略表现更好。
...
Method: 使用深度集成、蒙特卡洛Dropout等方法，在CIFAR-10和FFHQ图像数据集上测试多种集成规则，同时通过随机森林在表格数据上探索不同集成策略。

Result: 分数集成通常能改善分数匹配损失和模型似然，但未能一致提升FID等感知质量指标。在表格数据上发现某些集成策略优于其他方法。理论分析揭示了分数模型求和的性质。

Conclusion: 扩散模型集成在理论上具有优势，但在实际图像生成质量提升方面效果有限，这揭示了分数估计与图像质量之间的复杂关系，为模型组合技术提供了理论洞见。
...
### [73] [Low-Rank Key Value Attention](https://arxiv.org/abs/2601.11471)
*James O'Neill,Robert Clancy,Mariia Matskevichus,Fergal Reid*

Main category: cs.LG

TL;DR: LRKV是一种通过低秩残差共享KV投影来减少KV缓存内存的注意力机制改进方法，在保持完整token分辨率的同时，通过跨注意力头的冗余性实现内存优化。
...
Method: 提出低秩KV适应（LRKV）：修改多头注意力机制，每层使用共享的全秩KV投影，加上低秩、头特定的残差。这提供了从完全共享到完全独立注意力的连续权衡，是标准多头注意力的直接替代方案。

Result: 在大规模预训练实验中，LRKV相比标准注意力、MQA/GQA和MLA，实现了更快的损失下降、更低的验证困惑度和更强的下游任务性能。在2.5B规模下，使用约一半KV缓存即超越标准注意力，达到相同模型质量时训练计算减少20-25%。
...
### [74] [Extractive summarization on a CMOS Ising machine](https://arxiv.org/abs/2601.11491)
...
Result: 在CNN/DailyMail数据集上的实验结果显示，该流水线仅使用有限精度的整数耦合伊辛硬件即可生成高质量摘要。COBI相比暴力方法实现了3-4.5倍的运行时间加速，与软件Tabu搜索相当，同时能耗降低了2-3个数量级，同时保持了有竞争力的摘要质量。

Conclusion: 这些结果突显了在边缘设备上部署CMOS伊辛求解器进行实时、低能耗文本摘要的潜力，为资源受限环境中的高效摘要系统提供了新的硬件解决方案。
...
### [75] [Collaborative Continuum Robots: A Survey](https://arxiv.org/abs/2601.10721)
*Xinyu Li,Qian Tang,Guoxin Yin,Gang Zheng,Jessica Burgner-Kahrs,Cesare Stefanini,Ke Wu*

Main category: cs.RO

TL;DR: 该论文是一篇关于协作连续体机器人(CCRs)的综述，系统总结了这一新兴领域的研究进展，包括分类、设计、建模、规划与控制等方面。


<details>
  <summary>Details</summary>
Motivation: 连续体机器人(CRs)因其紧凑结构、固有柔顺性和灵活变形能力已在多个领域广泛应用。通过协调多个CRs形成协作连续体机器人(CCRs)，可以进一步提升任务适应性、工作空间、灵活性、负载能力和操作稳定性，具有显著优势。近年来，连续体机器人领域对这一新兴方向的兴趣持续增长，相关出版物也稳步增加。

Method: 该综述论文采用系统性综述方法：1) 将CCRs分为分离协作、辅助协作和并行协作三种协作模式并给出定义；2) 从系统架构层面全面概述最新进展；3) 系统总结每种模式在结构设计、建模、运动规划和控制方面的进展；4) 讨论当前挑战和未来机遇。

Result: 论文为CCRs研究提供了一个清晰的框架，系统梳理了三种协作模式下的技术进展，包括：分离协作模式下的独立操作协调、辅助协作模式下的主从辅助、并行协作模式下的协同负载等具体技术实现。

Conclusion: 协作连续体机器人(CCRs)是一个具有重要潜力的新兴研究方向，通过多机器人协作可以显著提升连续体机器人的性能。该综述为CCRs研究提供了系统框架，总结了当前进展并指出了未来发展方向和挑战。
...
### [76] [A Survey of Real-Time Support, Analysis, and Advancements in ROS 2](https://arxiv.org/abs/2601.10722)
*Daniel Casini,Jian-Jia Chen,Jing Li,Federico Reghenzani,Harun Teper*

Main category: cs.RO

TL;DR: 本文对ROS~2实时性研究进行了全面综述，涵盖其内部调度机制、架构、通信方式，以及学术界和工业界在提升ROS~2实时性能方面的关键贡献。


<details>
  <summary>Details</summary>
Motivation: ROS~2作为机器人应用的重要中间件框架，近年来在实时系统社区和工业界受到广泛关注。随着机器人系统对实时性要求的不断提高，需要对ROS~2的实时能力进行系统性分析和增强。本文旨在综述相关研究进展，为研究人员和从业者提供指导。

Method: 1. 详细描述ROS~2的内部调度机制和分层架构，包括与DDS通信和其他通信中间件的交互
2. 综述文献中的关键贡献：单线程和多线程执行器的时序分析、响应时间/反应时间/数据年龄等指标、不同通信模式
3. 讨论社区驱动的ROS~2运行时增强：新执行器算法设计、实时GPU管理、micro-ROS微控制器支持
4. 总结限制DDS通信延迟的技术、消息过滤器和分析工具
5. 引入分类法对综述内容进行系统化分类

Result: 本文提供了ROS~2实时性研究的全面概览，包括：1) ROS~2架构和调度机制的深入分析；2) 实时性能评估指标和方法；3) 社区开发的增强功能和工具；4) 系统化的分类框架。这些内容为理解和改进ROS~2的实时能力提供了系统性的参考。

Conclusion: ROS~2在实时机器人系统中的应用前景广阔，但需要进一步的研究和开发来满足严格的实时性要求。本文的综述为这一领域的研究提供了系统化的框架和方向，有助于推动ROS~2在实时系统中的发展和应用。
...
### [77] [Adaptive Sliding Mode Control for Vehicle Platoons with State-Dependent Friction Uncertainty](https://arxiv.org/abs/2601.10724)
*Rishabh Dev Yadav*
...
<details>
  <summary>Details</summary>
Motivation: 多机器人编队控制在车辆编队、有效载荷运输和监视等领域有广泛应用。主要挑战在于处理轮地间未知/不确定的摩擦力，这些摩擦力随路面条件、轮胎磨损和车速变化而变化。现有自适应控制器虽然能处理先验有界不确定性，但难以准确建模和识别状态依赖且无法先验有界的摩擦力。

Method: 提出一种新的自适应滑模控制器，采用两阶段过程：首先，运动学控制器根据期望轨迹计算期望速度；其次，动力学模型生成实现期望运动的命令。通过分离机器人的运动学和动力学，简化控制问题，实现更高效和鲁棒的轮式移动机器人控制。

Result: 该控制器能够处理未知和复杂的摩擦力行为，无需其参数和结构的先验知识，能够调节编队速度并维持预定义的机器人间距离，即使在存在外部扰动和不确定系统参数的情况下也能实现。

Conclusion: 所提出的自适应滑模控制器为轮式移动机器人车辆编队提供了一种有效的控制方案，能够处理复杂的摩擦力问题，提高编队控制的鲁棒性和适应性。
...
### [78] [Multi-Agent Formation Navigation Using Diffusion-Based Trajectory Generation](https://arxiv.org/abs/2601.10725)
...
Method: 使用扩散策略生成两个领导者中点作为刚性杆的轨迹，定义平面编队中的期望运动路径；跟随者使用基于局部坐标系相对位置的距约束编队控制器跟踪领导者

Result: 该方法产生平滑运动和低跟踪误差，大多数失败发生在狭窄无障碍空间或训练数据集中未包含的障碍物配置中

Conclusion: 仿真结果表明扩散模型在可靠的多智能体编队规划中具有潜力，为复杂环境中的编队控制提供了新方法
...
### [79] [Bidirectional Human-Robot Communication for Physical Human-Robot Interaction](https://arxiv.org/abs/2601.10796)
...
<details>
  <summary>Details</summary>
Motivation: 有效的物理人机交互需要系统既能适应用户偏好，又能透明展示其行为。当前系统缺乏自然语言交互和双向反馈机制，限制了用户体验的直观性和透明度。

Method: 使用大型语言模型（LLM）在规划运动轨迹和对话历史的上下文中解释用户指令隐含的轨迹修改（位置、速度、力）。系统提供双向语音反馈：确认修改结果或提出澄清问题。

Result: 18名老年人在三个辅助任务中的用户研究表明：参与者成功使用系统实时修改轨迹；双向反馈显著提高了交互性和透明度的评分，证明机器人语音反馈对更直观的用户体验至关重要。

Conclusion: BRIDGE系统通过自然语言交互和双向语音反馈实现了更直观、透明的物理人机交互，显著提升了用户体验，特别是在老年人群体的辅助任务中表现出色。
...
### [80] [Approximately Optimal Global Planning for Contact-Rich SE(2) Manipulation on a Graph of Reachable Sets](https://arxiv.org/abs/2601.10827)
*Simin Liu,Tong Zhao,Bernhard Paus Graesdal,Peter Werner,Jiuguang Wang,John Dolan,Changliu Liu,Tao Pang*

Main category: cs.RO

TL;DR: 提出了一种新的接触丰富操作规划范式，通过离线构建可达集图和在线规划，实现全局优化的接触丰富操作，相比现有方法显著降低任务成本并提高成功率。


<details>
  <summary>Details</summary>
Motivation: 接触丰富操作（CRM）相比仅使用末端执行器更高效自然，但现有基于模型的规划器主要关注可行性而非最优性，限制了CRM优势的充分发挥。

Method: 采用两阶段方法：离线阶段构建相互可达集图，每个集合包含从起始物体姿态和抓握可达的所有物体方向；在线阶段在该图上规划，计算并排序局部计划以实现全局优化的运动。

Result: 在具有挑战性的接触丰富任务中，该方法比领先规划器降低61%的任务成本，在250个查询中达到91%的成功率，并保持亚分钟级的查询时间。

Conclusion: 全局优化的接触丰富操作现在对于实际任务已变得实用，该方法为充分利用CRM优势提供了可行的解决方案。
...
### [81] [Is open robotics innovation a threat to international peace and security?](https://arxiv.org/abs/2601.10877)
*Ludovic Righetti,Vincent Boulanin*

Main category: cs.RO

TL;DR: 该论文探讨了机器人学领域开放获取带来的双重用途风险，提出了制定行业特定指导方针和监管的路线图。


<details>
  <summary>Details</summary>
Motivation: 机器人学领域的开放获取虽然促进了科学进步，但也加剧了双重用途风险，使国家和非国家行为者更容易将机器人技术用于军事和有害目的。与其他工程领域相比，机器人学缺乏专门的监管和指导方针。

Method: 提出一个包含四项实践的路线图：a) 负责任机器人学教育；b) 激励风险评估；c) 调节高风险材料的传播；d) 制定红线标准。

Result: 论文提出了一个系统性的框架，旨在帮助机器人学界建立自己的行业特定指导方针和可能的监管机制，以应对开放获取带来的双重用途风险。

Conclusion: 机器人学界需要制定自己的行业特定指导方针和监管框架，通过教育、风险评估、材料传播控制和红线制定等措施，在保持开放科学的同时负责任地管理双重用途风险。
...
### [82] [Where to Touch, How to Contact: Hierarchical RL-MPC Framework for Geometry-Aware Long-Horizon Dexterous Manipulation](https://arxiv.org/abs/2601.10930)
*Zhixian Xie,Yu Xiang,Michael Posa,Wanxin Jin*

Main category: cs.RO

TL;DR: 提出分层RL-MPC框架解决灵巧操作问题：高层RL预测接触意图（接触位置+子目标位姿），底层接触隐式MPC优化接触模式并规划动作，实现高效、鲁棒、零样本仿真到真实迁移


<details>
  <summary>Details</summary>
Motivation: 接触丰富的灵巧操作需要联合推理几何、运动学约束和非光滑接触动力学。端到端视觉运动策略需要大量数据、仿真到真实迁移差、跨任务泛化弱。灵巧操作本质上是分层的：高层决定接触位置和物体运动，底层通过接触动力学实现计划。

Method: 分层RL-MPC框架：高层RL策略预测接触意图（物体表面接触位置+接触后物体级子目标位姿）。基于此意图，底层接触隐式MPC优化局部接触模式，利用接触动力学重新规划，生成驱动物体朝向子目标的鲁棒机器人动作。

Result: 在非抓取任务（几何泛化推动和物体3D重定向）上评估，实现接近100%成功率，数据需求大幅减少（比端到端基线少10倍），高度鲁棒性能，零样本仿真到真实迁移。

Conclusion: 通过将灵巧操作分解为几何/运动学规划和接触动力学执行的分层结构，提出的框架克服了端到端方法的局限性，实现了高效学习、鲁棒性能和零样本仿真到真实迁移。
...
### [83] [Crane Lowering Guidance Using a Attachable Camera Module for Driver Vision Support](https://arxiv.org/abs/2601.11026)
*HyoJae Kang,SunWoo Ahn,InGyu Choi,GeonYeong Go,KunWoo Son,Min-Sung Kang*

Main category: cs.RO

TL;DR: 提出一种基于可附着摄像头的起重机负载降落引导系统，通过实时图像传输解决操作员视线被遮挡的问题


<details>
  <summary>Details</summary>
Motivation: 起重机在降落阶段，负载会遮挡操作员对降落点的视线，传统依赖地面人员口头或手势指导的方式存在安全隐患

Method: 设计可附着摄像头模块（包含单板计算机、电池和紧凑型摄像头），通过吸盘直接安装在负载上，实时采集和处理负载正下方地面图像，生成安装引导信息并传输到主机计算机

Result: 初步实验通过将模块附着在测试物体上，证实了实时图像采集和传输的可行性

Conclusion: 该系统有潜力通过为起重机操作员提供隐藏降落区域的即时视觉参考，显著提高建筑工地的安全性
...
### [84] [H-AIM: Orchestrating LLMs, PDDL, and Behavior Trees for Hierarchical Multi-Robot Planning](https://arxiv.org/abs/2601.11063)
...
<details>
  <summary>Details</summary>
Motivation: 在具身人工智能中，异构机器人团队执行高级指令的长期任务仍面临挑战。大型语言模型在指令解析和初步规划方面有潜力，但在长期推理和动态多机器人协调方面存在局限，需要新的规划框架来解决这些问题。

Method: 提出H-AIM三层级联架构：1) 利用LLM解析指令并生成PDDL问题描述，将命令转化为形式化规划问题；2) 结合LLM的语义推理能力和经典规划器的搜索能力，生成优化的动作序列；3) 将规划结果编译为行为树进行反应式控制。通过共享黑板机制支持动态规模的异构机器人团队通信和状态同步。

Result: 在MACE-THOR基准数据集（包含8个不同家庭布局的42个复杂任务）上的实验结果显示，H-AIM相比最强基线LaMMA-P，任务成功率从12%提升至55%，目标条件召回率从32%提升至72%，性能提升显著。

Conclusion: H-AIM框架通过结合LLM的语义理解能力和经典规划器的搜索能力，有效解决了异构机器人团队执行长期任务的规划问题，显著提升了任务执行的成功率和可靠性。
...
### [85] [A3D: Adaptive Affordance Assembly with Dual-Arm Manipulation](https://arxiv.org/abs/2601.11076)
...
Conclusion: A3D框架通过学习自适应可供性和使用交互反馈动态调整策略，成功解决了机器人双臂协作家具装配中的几何泛化和长时程适应问题
...
### [86] [Adaptive Monitoring of Stochastic Fire Front Processes via Information-seeking Predictive Control](https://arxiv.org/abs/2601.11231)
*Savvas Papaioannou,Panayiotis Kolios,Christos G. Panayiotou,Marios M. Polycarpou*

Main category: cs.RO

TL;DR: 提出一种自适应监测野火火线的移动智能体控制方法，将感知、估计和控制集成到随机最优控制框架中，通过信息寻求预测控制实现渐进最优策略。


<details>
  <summary>Details</summary>
Motivation: 现有野火监测方法通常将感知、估计和控制分开处理，要么施加线性高斯假设来建立最优性，要么依赖缺乏明确性能保证的近似和启发式方法。需要一种能够无缝集成这三个环节并具有性能保证的方法。

Method: 1) 将火线监测任务形式化为随机最优控制问题；2) 为随机非线性椭圆增长火线模型推导最优递归贝叶斯估计器；3) 将非线性随机控制问题转化为有限时域马尔可夫决策过程；4) 设计基于下置信界自适应搜索算法的信息寻求预测控制律，该算法具有渐进收敛到最优策略的特性。

Result: 提出了一个完整的野火监测框架，包括：1) 随机最优控制问题形式化；2) 针对一类火线模型的最优递归贝叶斯估计器；3) 将问题转化为MDP的方法；4) 具有渐进收敛保证的自适应搜索控制算法。

Conclusion: 该方法通过将感知、估计和控制集成到统一的随机最优控制框架中，克服了现有方法的局限性，提供了具有明确性能保证的自适应野火监测解决方案，能够渐进收敛到最优监测策略。
...
### [87] [VLAgents: A Policy Server for Efficient VLA Inference](https://arxiv.org/abs/2601.11250)
...
<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作模型的快速兴起对机器人学产生重大影响，但其部署仍然复杂，主要由于分布式设置中的碎片化接口和固有通信延迟问题。
...
Conclusion: VLAgents通过统一的协议和自适应通信机制，有效解决了VLA模型部署中的接口碎片化和通信延迟问题，为机器人控制提供了高效可靠的策略服务器解决方案。
...
### [88] [Distributed Control Barrier Functions for Safe Multi-Vehicle Navigation in Heterogeneous USV Fleets](https://arxiv.org/abs/2601.11335)
*Tyler Paine,Brendan Long,Jeremy Wenger,Michael DeFilippo,James Usevitch,Michael Benjamin*

Main category: cs.RO

TL;DR: 论文提出了一种用于异构无人船队碰撞避免的分布式安全控制滤波器方法，结合控制障碍函数理论和COLREGS规则，在模拟和真实实验中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 异构无人船队碰撞避免面临挑战，因为不同平台的决策过程和控制器存在差异，且实时共享轨迹和控制值存在限制。此外，需要处理包括有人船在内的其他接触物的最坏情况行为。

Method: 采用控制障碍函数理论开发分布式安全控制滤波器，为每艘自主车辆添加控制滤波器，假设其他接触物（包括有人船）采取最坏情况行为。将最坏情况CBF方法与COLREGS行为方法进行比较，并在模拟和真实实验中验证。

Result: 模拟和真实世界实验表明，该方法在多种平台上有效，对人类操作员的不合作行为具有鲁棒性。结合CBF方法和COLREGS行为能够实现最佳的安全性和效率。

Conclusion: 提出的分布式安全控制滤波器方法解决了异构无人船队碰撞避免的挑战，结合控制障碍函数理论和COLREGS规则的综合方法在实际应用中表现出良好的安全性和效率。
...
### [89] [ACoT-VLA: Action Chain-of-Thought for Vision-Language-Action Models](https://arxiv.org/abs/2601.11404)
*Linqing Zhong,Yi Liu,Yifei Wei,Ziyu Xiong,Maoqing Yao,Si Liu,Guanghui Ren*

Main category: cs.RO

TL;DR: 提出ACoT-VLA架构，通过动作思维链范式，在动作空间直接推理，提升机器人操作任务的执行精度


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型通常依赖VLM嵌入直接转换多模态输入为动作，或使用语言子任务预测、视觉目标图像合成等间接中间推理。但这些中间推理形式在传达精确动作执行所需的完整、细粒度信息方面存在固有局限性。作者认为最有效的推理形式应直接在动作空间进行思考。

Method: 提出ACoT-VLA架构，实现动作思维链范式。包含两个互补组件：显式动作推理器(EAR)提出粗粒度参考轨迹作为显式动作级推理步骤；隐式动作推理器(IAR)从多模态输入的内部表示中提取潜在动作先验。两者共同形成ACoT，为下游动作头提供条件，实现基于推理的策略学习。

Result: 在真实世界和仿真环境中进行广泛实验，在LIBERO、LIBERO-Plus和VLABench基准上分别达到98.5%、84.1%和47.4%的性能表现，证明了方法的优越性。

Conclusion: 动作思维链范式通过在动作空间直接推理，能够提供更有效、更精确的中间表示，显著提升VLA模型在机器人操作任务中的性能。ACoT-VLA架构通过显式和隐式动作推理器的协同工作，实现了这一范式。
...
Method: 提出约束性时间分层架构(CTHA)，包含三个关键约束：1)消息契约约束，通过类型化摘要、计划和策略包形式化层间信息流；2)权威流形约束，根据时间范围限定各层决策空间；3)仲裁器解决约束，保证多层决策的无冲突组合。
...
Method: 提出LMEE框架统一智能体的探索认知和决策行为。构建LMEE-Bench数据集和基准，包含多目标导航和基于记忆的问答任务。提出MemoryExplorer方法，通过强化学习微调多模态大语言模型，采用包含动作预测、边界选择和问答的多任务奖励函数，鼓励主动记忆查询。

Result: 与最先进的具身探索模型进行广泛实验对比，证明该方法在长时程具身任务中取得显著优势，提升了智能体的记忆回忆和主动探索能力。

Conclusion: LMEE框架和MemoryExplorer方法通过统一探索认知与决策行为，结合主动记忆查询机制，有效促进了具身智能体的终身学习能力，在长时程复杂任务中表现出色。
...
TL;DR: 该论文使用基于启发式的趋势模型研究复杂产品创新过程，通过简单趋势（递增、递减、恒定）作为最小信息强度量化器，避免依赖数值或粗糙集。


<details>
  <summary>Details</summary>
Motivation: 研究复杂产品创新过程需要有效的建模方法，传统方法可能过度依赖数值数据或复杂集合理论。作者寻求一种更简洁、信息强度最小化的量化方式，通过简单趋势来捕捉创新过程的关键动态特征。

Method: 采用基于启发式的趋势模型，每个启发式通过简单趋势（递增、递减、恒定）表达。将趋势模型的解定义为具有可能转换的场景集合，用转换图表示。系统的任何可能未来或过去行为都可以通过图中的路径描绘。

Result: 提出了一种新的复杂产品创新过程建模框架，使用趋势作为最小信息强度量化器，避免了数值依赖。通过转换图表示场景间的可能转换，为系统行为分析提供了可视化工具。

Conclusion: 基于启发式的趋势模型为复杂产品创新过程研究提供了一种简洁有效的建模方法，通过最小信息强度量化器和转换图表示，能够捕捉系统动态特征并支持未来行为预测。
...
TL;DR: ARC-AGI-2竞赛显示AI抽象推理进展，但当前方法仍受限于知识覆盖而非真正的泛化能力，催生了新的基准污染问题。


<details>
  <summary>Details</summary>
Motivation: 分析ARC-AGI-2竞赛结果，研究2025年涌现的优化循环方法，探讨当前AI推理能力的根本限制，并为下一代基准ARC-AGI-3做准备。

Method: 对ARC Prize 2025竞赛进行系统性调查，分析1455个团队的154个参赛作品，特别关注优化循环方法（进化程序合成和商业AI系统应用层优化），同时评估四大前沿AI实验室的公开模型表现。

Result: 竞赛最高得分24%，论文提交量翻倍至90篇，优化循环成为主导方法。前沿AI实验室开始将ARC-AGI作为行业标准基准，但分析显示当前推理性能仍受限于知识覆盖，导致新的基准污染形式。

Conclusion: 优化循环方法推动了AGI进展，但当前AI推理能力本质上仍受知识覆盖限制。ARC-AGI-3将引入需要探索、规划、记忆、目标获取和对齐能力的交互式推理挑战，以推动真正的抽象推理发展。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索多模态推理中的数据策展问题，特别是在固定模型和训练协议的情况下，如何通过数据集选择来提升性能。DCVLR挑战赛为此提供了理想的实验框架。

Method: 研究方法包括：1）使用基于Walton多模态冷启动的紧凑策展数据集；2）在固定训练方案下进行数据集选择实验；3）通过赛后消融研究分析不同数据策展策略（难度选择、数据集大小、多样性启发式、合成增强）的影响。

Result: 主要结果：1）基于难度的样本选择是性能提升的主要驱动因素；2）增加数据集大小不能可靠提高平均准确率，但能减少运行间方差；3）常用的多样性和合成增强启发式方法没有额外益处，反而经常降低性能；4）该研究在DCVLR挑战赛中获得了第一名。

Conclusion: 结论表明DCVLR是一个饱和机制评估，强调了对齐和难度在数据高效多模态推理中的核心作用。研究结果为多模态推理的数据策展提供了重要见解，特别是在资源受限或需要高效数据利用的场景中。
...
TL;DR: ReCreate是一个经验驱动的自动领域智能体创建框架，通过智能体交互历史学习，采用智能体即优化器范式，在多个领域超越人工设计和现有自动方法


<details>
  <summary>Details</summary>
Motivation: 当前大多数实际智能体仍由人工设计，任务差异大导致构建成本高。现有自动智能体生成方法将其视为黑盒过程，仅依赖最终性能指标，忽略了成功/失败的关键证据，且计算成本高

Method: 提出ReCreate框架，采用智能体即优化器范式，包含三个核心组件：1) 经验存储与检索机制，用于按需检查；2) 推理-创建协同管道，将执行经验映射到脚手架编辑；3) 分层更新，将实例级细节抽象为可重用领域模式

Result: 在多个不同领域的实验中，ReCreate始终优于人工设计的智能体和现有自动智能体生成方法，即使从最小种子脚手架开始也能取得良好效果

Conclusion: ReCreate通过系统利用智能体交互历史中的具体信号，成功解决了自动创建和适应领域智能体的挑战，为智能体自动生成提供了更高效、可解释的解决方案
...
### [96] [Do We Always Need Query-Level Workflows? Rethinking Agentic Workflow Generation for Multi-Agent Systems](https://arxiv.org/abs/2601.11147)
*Zixu Wang,Bingbing Xu,Yige Yuan,Huawei Shen,Xueqi Cheng*

Main category: cs.AI

TL;DR: SCALE框架通过任务级工作流生成和基于自预测的评估替代完整执行验证，在保持性能的同时大幅降低token消耗


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的多智能体系统通常通过任务级或查询级工作流协调，但两者的相对成本和效益不明确，且完整执行验证token成本高且不可靠

Method: 提出SCALE框架：1）任务级工作流生成，少量最佳任务级工作流可覆盖多数查询；2）自预测优化器通过少量样本校准进行评估，替代完整验证执行

Result: SCALE在多个数据集上平均性能仅下降0.61%，同时将总体token使用量减少高达83%，保持竞争力的同时显著降低成本

Conclusion: 查询级工作流生成并非总是必要，任务级工作流结合自预测评估可有效平衡性能与成本，为多智能体系统提供高效解决方案
...
### [97] [TANDEM: Temporal-Aware Neural Detection for Multimodal Hate Speech](https://arxiv.org/abs/2601.11178)
...
Conclusion: 即使在复杂的多模态环境中，结构化、可解释的对齐也是可实现的，为下一代透明且可操作的在线安全审核工具提供了蓝图。该方法展示了通过结构化推理而非简单分类来处理多模态仇恨检测的潜力。
...
### [98] [Beyond Model Scaling: Test-Time Intervention for Efficient Deep Reasoning](https://arxiv.org/abs/2601.11252)
*Qianyue Wang,Jinwu Hu,Yufeng Wang,Huanxiang Lin,Bolin Chen,Zhiquan Wen,Yaofo Chen,Mingkui Tan*

Main category: cs.AI

TL;DR: Think-with-Me是一种测试时交互式推理范式，通过在推理过程中引入外部反馈干预，解决大型推理模型中存在的过度思考和推理偏差问题，在有限上下文窗口中实现准确性与推理长度的更好平衡。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在多步推理方面表现出色，但存在推理效率低下的问题，如过度思考和推理偏差，这些不必要的推理增加了计算成本并降低了性能。现有的高效推理方法以闭环方式运行，缺乏外部干预机制来指导推理过程。

Method: 提出Think-with-Me交互式推理范式，核心洞察是过渡连词可作为自然干预点，标志着自我验证或探索阶段。方法在推理过程中暂停以获取外部反馈，通过多标准评估（合理性和完整性）生成反馈，使用Group Relative Policy Optimization训练目标模型适应交互模式，自适应地延长或终止推理以减少冗余。

Result: 在有限上下文窗口下，Think-with-Me实现了准确性与推理长度的优越平衡。在AIME24上，Think-with-Me在8K窗口下比QwQ-32B准确率提高7.19%，同时平均推理长度减少81%。该范式在安全和创造性任务中也显示出益处。

Conclusion: Think-with-Me通过引入外部反馈干预，有效解决了大型推理模型的效率问题，在保持准确性的同时显著减少推理长度，为高效推理提供了新的交互式范式，在数学推理、安全和创造性任务中均有应用价值。
...
### [99] [XChoice: Explainable Evaluation of AI-Human Alignment in LLM-based Constrained Choice Decision Making](https://arxiv.org/abs/2601.11286)
*Weihong Qi,Fan Huang,Rasika Muralidharan,Jisun An,Haewoon Kwak*

Main category: cs.AI

TL;DR: XChoice是一个可解释的框架，用于评估约束决策中AI与人类的对齐性，超越传统准确性指标，通过机制建模分析决策因素重要性、约束敏感性和权衡取舍。


<details>
  <summary>Details</summary>
Motivation: 传统AI评估主要关注结果一致性（如准确率、F1分数），但缺乏对决策机制的深入理解。需要开发能够揭示AI与人类在约束决策中内在对齐程度的解释性框架。

Method: XChoice采用基于机制的决策模型，拟合人类数据和LLM生成的决策，恢复可解释参数（决策因素相对重要性、约束敏感性、隐含权衡）。通过比较这些参数向量来评估对齐性，并在美国时间使用调查数据上进行验证，进行不变性分析和RAG干预评估。

Result: 在美国日常时间分配案例中，XChoice揭示了模型和活动间的异质性对齐，以及黑人和已婚群体中的显著不对齐。框架通过不变性分析验证了鲁棒性，并通过RAG干预展示了针对性缓解效果。

Conclusion: XChoice提供了基于机制的度量标准，能够诊断不对齐问题并支持超越表面结果匹配的知情改进，为AI与人类对齐评估提供了更深入的分析工具。
...
### [100] [AstroReason-Bench: Evaluating Unified Agentic Planning across Heterogeneous Space Planning Problems](https://arxiv.org/abs/2601.11354)
...
<details>
  <summary>Details</summary>
Motivation: 现有智能体基准主要关注符号化或弱接地环境，缺乏对物理约束现实世界领域的评估。空间规划问题具有高风险、异构目标、严格物理约束和长时程决策等特点，需要专门的评估基准。

Method: 引入AstroReason-Bench基准，整合地面站通信和敏捷地球观测等多种调度机制，提供统一的智能体导向交互协议，并评估多种最先进的开放和闭源智能体LLM系统。

Result: 当前智能体在空间规划问题上表现显著低于专业求解器，突显了通用规划器在现实约束下的关键局限性。
...
### [101] [Hyperparameter Optimization of Constraint Programming Solvers](https://arxiv.org/abs/2601.11389)
*Hedieh Haddad,Thibault Falque,Pierre Talbot,Pascal Bouvry*

Main category: cs.AI

TL;DR: 提出"探针与求解"两阶段框架，用于约束规划求解器的自动化超参数优化，通过贝叶斯优化在有限时间内显著提升求解性能


<details>
  <summary>Details</summary>
Motivation: 约束规划求解器的性能对超参数配置高度敏感，手动调优需要专业知识且耗时，需要自动化方法来优化求解器配置

Method: 提出两阶段框架：探针阶段使用可配置的超参数优化方法（贝叶斯优化和汉明距离搜索）探索参数空间，求解阶段使用最佳配置解决问题；在CPMpy库中实现并比较两种优化方法

Result: 贝叶斯优化显著优于默认配置：ACE求解器在25.4%实例中提升质量，57.9%持平；Choco求解器在38.6%实例中表现更优；贝叶斯优化始终优于汉明距离搜索

Conclusion: 探针与求解算法提供了一种实用、资源感知的约束求解器调优方法，能跨多种问题类型实现稳健改进，模型化探索优于简单局部搜索
...
### [102] [Exploring LLM Features in Predictive Process Monitoring for Small-Scale Event-Logs](https://arxiv.org/abs/2601.11468)
*Alessandro Padella,Massimiliano de Leoni,Marlon Dumas*

Main category: cs.AI

TL;DR: 本文扩展了基于LLM的预测性过程监控框架，从仅预测总时间扩展到多KPI预测，在数据稀缺场景下（仅100条轨迹）LLM超越基准方法，并利用其先验知识和训练轨迹内部相关性进行高阶推理。


<details>
  <summary>Details</summary>
Motivation: 预测性过程监控旨在预测进行中过程的结果，传统方法使用机器学习和深度学习架构。本文动机是扩展先前基于LLM的预测性过程监控框架，从仅关注总时间预测扩展到全面评估其泛化性、语义利用和推理机制，并涵盖多个关键绩效指标。

Method: 扩展了基于LLM的预测性过程监控框架，通过提示机制进行预测。方法包括：1）评估框架的泛化性；2）分析语义利用能力；3）研究推理机制；4）扩展到多个关键绩效指标（总时间和活动发生预测）。在三个不同事件日志上进行实证评估。

Result: 在数据稀缺设置（仅100条轨迹）下，LLM在总时间和活动发生预测两个KPI上都超越了基准方法。实验表明LLM既利用其先验知识，也利用训练轨迹间的内部相关性。LLM不简单复制现有预测方法，而是进行高阶推理来生成预测。

Conclusion: 基于LLM的预测性过程监控框架在数据稀缺场景下表现出色，能够有效利用先验知识和数据内部相关性，并通过高阶推理机制生成预测，为多KPI预测提供了有效解决方案。
...
### [103] [Health Facility Location in Ethiopia: Leveraging LLMs to Integrate Expert Knowledge into Algorithmic Planning](https://arxiv.org/abs/2601.11479)
...
<details>
  <summary>Details</summary>
Motivation: 埃塞俄比亚卫生部升级卫生站以改善基本医疗服务可及性，但资源有限需要优先选择升级设施。传统优化方法需要明确量化目标，而利益相关者标准通常用自然语言表达难以形式化，需要结合专家知识与优化技术

Method: 提出LEG（大语言模型与扩展贪心）框架：结合可证明近似算法优化人口覆盖，使用LLM驱动的迭代细化纳入人机对齐，确保解决方案反映专家定性指导同时保持覆盖保证

Result: 在埃塞俄比亚三个地区的真实数据实验证明该框架有效，能够为公平、数据驱动的卫生系统规划提供信息

Conclusion: LEG框架成功整合专家知识与优化技术，解决了卫生系统规划中量化目标与定性偏好之间的鸿沟，为资源有限环境下的公平卫生服务规划提供了可行方案
...
### [104] [BoxMind: Closed-loop AI strategy optimization for elite boxing validated in the 2024 Olympics](https://arxiv.org/abs/2601.11492)
*Kaiwen Wang,Kaili Zheng,Rongrong Deng,Qingmin Fan,Milin Zhang,Zongrui Li,Xuesi Zhou,Bo Han,Liren Chen,Chenyi Guo,Ji Wu*

Main category: cs.AI

TL;DR: BoxMind：一个用于拳击战术分析的闭环AI专家系统，通过定义原子击打事件和分层技术战术指标，结合图基预测模型，将比赛视频转化为可执行的战术建议，在2024巴黎奥运会上验证有效。


<details>
  <summary>Details</summary>
Motivation: 拳击等格斗类运动在AI驱动分析方面发展不足，主要由于动作动态复杂且缺乏结构化战术表示。需要将非结构化视频数据转化为战略智能，弥合计算机视觉与竞技体育决策支持之间的差距。

Method: 1. 定义具有精确时间边界、空间和技术属性的原子击打事件；2. 将比赛视频解析为18个分层技术战术指标；3. 提出基于图的预测模型，融合显性技术战术特征与可学习的时变潜在嵌入；4. 将比赛结果建模为技术战术指标的可微函数，将获胜概率梯度转化为可执行的战术调整。

Result: 1. 结果预测模型在BoxerGraph测试集上达到69.8%准确率，在奥运比赛上达到87.5%准确率；2. 系统生成的战略建议达到与人类专家相当的水平；3. 在2024巴黎奥运会闭环部署中，直接助力中国国家队获得3金2银的历史性成绩。
